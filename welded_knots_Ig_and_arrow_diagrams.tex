...
\]
Let us determine the functional \(\xi\) in terms of \(x\) and \(\psi\). There is only one type of nonzero trivalent vertex in \(\mathcal{A}_{w}\), so this is just a rotated version and it is represented by the same tensor, just viewed from a different component. That tensor is \([\cdot, \cdot] \in \mathfrak{g}^{\ast} \otimes \mathfrak{g}^{\ast} \otimes \mathfrak{g}\), so \(\xi\) satisfies the equation \([\cdot, \cdot] = x^{\ast} \otimes \xi \otimes \phi^{\ast}\), whereby
...
The \ref{eq:AS} relation gives \([\psi \oplus 0, 0 \oplus y] = -\operatorname{ad}^{\ast}_{y}(\psi) \oplus 0\).

The total Lie algebra structure on \(\mathfrak{g}^{\ast} \rtimes \mathfrak{g}\) is given by the following definition.
...
\begin{definition}
	The \(2\)-coloured prop \(I\) is the prop generated by the objects \(X\) and \(Y\), and the morphisms \(c\), the formal relations from the bracket in \(\mathfrak{g}\), the formal relations from the other bracket.
\end{definition}
