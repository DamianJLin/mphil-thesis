...
\lettrine{\libertineInitialGlyph{W}}{elded} knots, or \(w\)-knots are a diagrammatic generalisation of knots that relate to codimension two knotted objects in \(\mathbb{R}^{4}\), rather than \(\mathbb{R}^{3}\).
...
\begin{definition}
	Let \(X = (X, \beta)\) be a Lie algebra object in the \(k\)-linear symmetric monoidal category \(\mathcal{C}\). We define a \(2\)-coloured prop \(\mathbf{T}(X, \beta)\), or just \(\mathbf{T}_{X}\).

	Its generating objects are \(X\) which we write as \(\mathbf{1}\), and another object \(\mathbf{1}^{\vee}\). Define
	\[\mathbf{I} = \mathbf{1} \otimes \mathbf{1}^{\vee} \in \mathbf{T}_{X}\]
	...
	and
	\[\alpha: \mathbf{1}^{\vee} \otimes \mathbf{1} \longrightarrow \mathbf{1}^{\vee}\]
	...
	\begin{enumerate}
		...
		\item Let \(\sigma'\) be the extension of the action of the permutation \(\sigma \in S_{2}\) to \(S_{3}\).
			\[\sigma'(\beta_{13}) = \alpha_{23}, \sigma'(\alpha_{23}) = -\alpha_{14}, \sigma(-\alpha_{14}) = \beta_{13}\]
\end{enumerate}
\end{definition}
...
\begin{shaded}
	\textbf{Problem}: Unless we connect the bracket to the coadjoint somehow there's no way we can get an isomorphism (same diagram will have multiple correspoding tensor elements). However it seems without working in the metric envelope, that's not possible.

	One possible way to fix it: have 3 morphisms and assert that the extension of the \(S_{n}\) action permutes them... But then this feels like we should be working in a 1-coloured prop.
\end{shaded}
