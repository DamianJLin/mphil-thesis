...
The following category fills the role that was played by the prop \(\mathbf{E}_{m}(\mathcal{O}_{\text{Lie}})\) in the classical setting. Since the morphisms in the welded case should reflect the structure of \(I(\mathfrak{g})\), and the bracket on \(I(\mathfrak{g})\) decomposes depending on whether \(x \in I(\mathfrak{g})\) is in \(\mathfrak{g}\) or \(\mathfrak{g}^{\ast}\), the category ought to have two objects, \(\mathbf{1}\) and \(\mathbf{1}^{\vee}\) and be linear.

\begin{definition}
	\label{def:2-coloured-casimir-prop}
	We define a symmetric monoidal category \(\mathbf{T}_{m}\). It is a \(k\)-linear category and it is generated additively and monoidally by the objects \(\mathbf{1}\) and \(\mathbf{1}^{\vee}\). Denote the objects \(\mathbf{1}^{\otimes n}\) and \((\mathbf{1}^{\vee})^{\otimes n}\) as \(\mathbf{n}\) and \(\mathbf{n}^{\vee}\) respectively, as well as \((\mathbf{1}^{\vee})^{\otimes n} \otimes \mathbf{1}^{\otimes k}\) as \(\mathbf{n}^{\vee} + \mathbf{k}\).

	The generating morphisms of \(\mathbf{T}_{m}\) are given by
	\[
...
\begin{proof}
	...
	\]
	which is a consequence of the fact that morphisms in \(\mathbf{T}_{c}\) are direct sums of elements of \(\mathcal{O}_{\text{Lie}}\) (Definition \ref{def:2-coloured-casimir-prop}), and the equivariance axioms of the Lie operad in Definition \ref{def:operad-over-set}.
...
We obtain as a corollary the following description of \(\mathcal{A}_{w}\).
...
The proof follows the proof of Theorem \ref{thm:universal-casimir-lie-envelope-chord-diagrams-algebra-isomorphism}, but with \(\mathcal{A}\) replaced by \(\mathcal{A}_{w}\) and with \(\mathbf{1}\) replaced by \(\mathbf{I}\).

\begin{proof}
	We denote objects \(\mathbf{I}^{\otimes n}\) in \(\mathbf{T}_{c}\) as \(\mathbf{nI}\). Recall the definition of \(T(\mathcal{O}_{\text{Lie}}, \mathbf{I})\) as
	\begin{align*}
		T(\mathcal{O}_{\text{Lie}}, \mathbf{I})
		& = \bigoplus_{n \in \mathbb{N}} \mathcal{O}_{\text{Lie}}(n + 1) \otimes_{S_{n}} \Gamma(\mathbf{nI}) \\
		& = \bigoplus_{n \in \mathbb{N}} \mathcal{O}_{\text{Lie}}(n + 1) \otimes_{S_{n}} \mathbf{T}_{c}(\mathbf{0}, \mathbf{nI}).
	\end{align*}
	Just as in the proof of Theorem \ref{thm:universal-casimir-lie-envelope-chord-diagrams-algebra-isomorphism}, by Lemma \ref{lem:lie-operad-left-skewed-tree-normal-form}, the \(kS_{n}\)-action on \(\mathcal{O}_{\text{Lie}}\), any element \(t \in T(\mathcal{O}_{\text{Lie}}, \mathbf{I})\) may be uniquely written as
	\[
		t
		=
		\bigoplus_{n \in \mathbb{N}} L_{n + 1} \otimes_{S_{n}} A_{n}
	\]
	with \(L_{n + 1}\) the left-skewed bracket in \(\mathcal{O}_{\text{Lie}}(n + 1)\) and \(A_{n} \in \mathbf{T}_{c}(\mathbf{0}, \mathbf{nI})\). This defines a vector space isomorphism
	\[
		\begin{array}{rccc}
			& T(\mathcal{O}_{\text{Lie}}, \mathbf{I})
			& \longrightarrow
			& \displaystyle \bigoplus_{n \in \mathbb{N}} \mathbf{T}_{c}(\mathbf{0}, \mathbf{nI})\\
			& t
			& \longmapsto
