...
\begin{definition}
	A \textbf{welded knot} is an equivalence class of four-valent planar diagram, where each four-valent planar vertex is decorated by a vertex of one of the following types.
...
\begin{definition}
	Let \(X = (X, \beta)\) be a Lie algebra object in the \(k\)-linear symmetric monoidal category \(\mathcal{C}\). We define a \(2\)-coloured prop \(\mathbf{T}(X, \beta)\), or just \(\mathbf{T}(X)\).

	Its generating objects are \(X\) which we write as \(\mathbf{1}\), and another object \(\mathbf{1}^{\vee}\). Define
	\[\mathbf{I} = \mathbf{1} \otimes \mathbf{1}^{\vee} \in \mathbf{T}(X)\]
	and we denote \(\mathbf{I}^{\otimes n}\) as \(\mathbf{nI}\).

	For its generating morphisms, we have
	\[\beta: \mathbf{1} \otimes \mathbf{1} \longrightarrow \mathbf{1}\]
	and one further generating morphism
	\[\gamma: \mathbf{0} \longrightarrow \mathbf{I} \otimes \mathbf{I}\]
	satisfying some following conditions.

	\begin{enumerate}
		\item The morphism \(\beta\) satisfies the relations satisfied by \(\beta\) in \(\mathcal{C}\).
		\item Let \(\zeta \in \mathbf{T}(\mathbf{1}^{\vee} \otimes \mathbf{1}^{\vee}, \mathbf{1}^{\vee})\) be the zero-morphism, and \(\gamma = \beta \otimes \zeta\). Therefore \(\mathbf{I} = (\mathbf{I}, \gamma)\) is a Lie algebra object in \(\mathbf{T}(X)\), so the following makes sense. For each \(f \in \mathcal{O}_{\text{Lie}}(n)\), the diagram
		% https://q.uiver.app/#q=
		\[\begin{tikzcd}
			{(\mathbf{n - 1})\mathbf{I}} && {(\mathbf{n + 1})\mathbf{I}} \\
			\\
			{(\mathbf{n + 1})\mathbf{I}} && {\mathbf{2I}}
		...
		\end{tikzcd}\]
		commutes. Here \(\sigma_{c}\) is the cyclic permutation in \(S_{2(n + 1)}\) sending \(0 \mapsto 2\). It acts blockwise cyclically on blocks of size \(2\) by the extension of the action of \(S_{n}\) on \(\mathbf{nI}\) to \(S_{n + 1}\).
\end{enumerate}
\end{definition}

\begin{theorem}
	The bialgebra \(\mathcal{A}_{w}\) of arrow diagrams is naturally isomorphic as a bialgebra to the external enveloping algebra \(U(\mathcal{O}_{\text{Lie}}, \mathbf{I})\).
\end{theorem}

\begin{proof}
	Similar to Theorem \ref{thm:universal-casimir-lie-envelope-chord-diagrams-isomorphism}.
\end{proof}
