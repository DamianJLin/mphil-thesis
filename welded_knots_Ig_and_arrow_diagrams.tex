...
The theorem of Hinich-Vaintrob in the classical case is the isomorphism between \(\mathcal{A}\) and the generalisation \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c})\) of a universal enveloping algebra. It is important to note the implicit dependence of \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c})\) (and also \(T(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c})\), from which it is defined) on the category \(\mathbf{E}_{c} \ni \mathbf{1}_{c}\). In Definition \ref{def:external-tensor-algebra}, this is hidden in the notation \(\Gamma(\mathbf{n}_{c}) = \mathbf{E}_{c}(\mathbf{0}, \mathbf{n})\).

For a welded version of this statement, the corresponding associative algebra should generalise \(\mathcal{U}(I\mathfrak{g})\) and therefore must contain associative products of elements of \(\mathfrak{g}\) and \(\mathfrak{g}^{\ast}\). This nececcitates the definition of a categorical generalisation of \(\mathcal{U}(I\mathfrak{g})\).

\begin{definition}
	\label{def:external-bivariant-tensor-algebra}
	The \textbf{external bivariant tensor algebra \(T_{\ast}(\mathcal{O}, A)\) of the rigid algebra \(A\) over the operad \(\mathcal{O}\)} is the algebra
	\[T_{\ast}(\mathcal{O}, A) = \bigoplus_{n \in \mathbb{N}} \left( \mathcal{O}(n + 1) \otimes_{S_{n}} \bigoplus_{w \in \{A, A^{\vee}\}^{\otimes n}}\Gamma(w) \right).\]

	Note that since \(A\) is rigid, it has a unique dual, so this is well-defined.
\end{definition}

Similarly, we generalise the external universal enveloping algebra to allow associative products of elements of an algebra or its dual.

\begin{definition}
	\label{def:external-bivariant-enveloping-algebra}
	The \textbf{external bivariant tensor algebra \(U_{\ast}(\mathcal{O}, A)\) of the rigid algebra \(A\) over the operad \(\mathcal{O}\)} is the quotient of \(T_{\ast}(\mathcal{O}, A)\) by the weakest equivalence relation making the following diagram commute for all \(n, m_{1}, \dots, m_{n} \in \mathbb{N}\).
\end{definition}
