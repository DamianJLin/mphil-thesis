...
The proof follows the proof of Theorem \ref{thm:universal-casimir-lie-envelope-chord-diagrams-isomorphism}, but with \(\mathcal{A}\) replaced by \(\mathcal{A}_{w}\) and with \(\mathbf{1}\) replaced by \(\mathbf{I}\).
...
\begin{proof}
	...
	\end{align*}
	Just as in the proof of Theorem \ref{thm:universal-casimir-lie-envelope-chord-diagrams-isomorphism}, by Lemma \ref{lem:lie-operad-left-skewed-tree-normal-form}, the \(kS_{n}\)-action on \(\mathcal{O}_{\text{Lie}}\), any element \(t \in T(\mathcal{O}_{\text{Lie}}, \mathbf{I})\) may be uniquely written as
	\[
		t
		=
