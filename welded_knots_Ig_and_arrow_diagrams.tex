...
There was no need to define long knots earlier because in the classical case, long knots and knots are in bijection with each other and the theory is the same for both long and regular knots. In the welded case however the long theory has the advantage of eliminating the global orientation reversal move.
...
\begin{proof}
	...
	\end{align*}
	By Lemma \ref{lem:lie-operad-left-skewed-tree-normal-form}, the \(kS_{n}\)-action on \(\mathcal{O}_{\text{Lie}}\), any element \(t \in T(\mathcal{O}_{\text{Lie}}, \mathbf{I})\) may be uniquely written as
	\[
		t
		=
	...
	\end{tikzcd}\]
	Note that in the case \(n = 1, m_{1} = 2\) on the object \(y \otimes y \otimes X\) for arbitrary \(X \in \mathbf{T}_{\varepsilon}(\mathbf{0}, \mathbf{2I})\), this enforces the regular \ref{eq:STU} of Example \ref{ex:stu-in-external-enveloping-algebra} as well as two variants, (corresponding to the three variants of \ref{eq:STU} in \(\mathcal{A}_{w}\)) and also the \ref{eq:TC} relation. The map \(f\) therefore factors through the quotient to \(U(\mathcal{O}, \mathbf{I})\).

	The map \(f\) is clearly surjective --- every arrow diagram has an underlying chord diagram, which can be presented by a graph whose edges do not have local minima. The arrow diagram can therefore be presented with only directed Casimirs (caps), and trivalent two-in-one-out vertices. A morphism in \(\mathbf{T}_{\varepsilon}(\mathbf{0}, \mathbf{n})\) is built by presenting the two-in-one-out vertices by morphisms
	\[
		{\scriptsize
		\def\svgscale{0.38}
		\adjustbox{valign=c}{\input{graphics/two_coloured_bracket.pdf_tex}}
	...
	\]
	and cups by the morphism \(c: \mathbf{0} \to \mathbf{I} \otimes \mathbf{I}\).
