...
\end{shaded}
Of course, in the \ref{eq:TC} case, this is only true becasue \(\lambda = 0\).)

So, the correct associative algebra target for the weight system is still the Universal enveloping algebra of some Lie bialgebra. From the relations above we can determine the structure of this Lie algebra.

\begin{shaded}
	By the regular \ref{eq:STU}, it should have the bracket of \(\mathfrak{g}\) on the \(\mathfrak{g}\) part. By \ref{eq:TC} it should have the zero bracket on the \(\mathfrak{g}^{\ast}\) part. By the other two brackets, it should have the coadjoint action (which needs to be defined).
\end{shaded}

\begin{definition}
	Let \(\mathfrak{g}\) be a finite dimensional Lie algebra.

	The \textbf{cocommutative double} of a finite-dimensional Lie algebra \(\mathfrak{g}\), denoted \(I\mathfrak{g}\), is the Lie algebra 
\end{definition}
