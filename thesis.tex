...
\begin{document}
        ...
        \begin{definition}[Vassiliev Invariant] \label{def:vassiliev_invairant}
                ...
                \[V^{(n + 1)}\biggl( \underbrace{\singular \cdots \singular}_{{\scriptscriptstyle n + 1}} \biggr) = 0.\]
        ...
        \[\underbrace{\singular \cdots \singular}_{{\scriptscriptstyle n}}\]
        could represent. Exactly what describes a `kind' of \(n\)-singular knot will turn out to be a chord diagram, but note: other than this kind, the \(n\)th derivative of an \(n\)-singular knot does not depend on the \(n\)-singular knot, and in that sense it is a constant.
        ...
        This space \(\mathcal{K}^{0}_{n} / \delta\mathcal{K}^{0}_{n + 1} = \mathbf{A}_{m}\) is very important for describing Vassiliev invariants.

        \begin{proposition}
                To every Vassiliev invariant \(v\) of order \(n\), we can assign a linear functional on \(\mathbf{A}_{m}\), that is an element of \((\mathbf{A}_{m})^{\ast}\).
        \end{proposition}
        \begin{proof}
        ...
        \begin{proposition}
                The vassiliev invariant above is completely determined by the linear functional when evaluataed on \(m\)-singular knots; i.e. something related to the 'derivative' map is injective on these knots, maybe?
        \end{proposition}
        \begin{proof}
                Computing \(v\) shows it depends only on its value on the chord diagram of the \(m\)-singular knot. So, specifying a value on the chord diagram determines \(v(K)\).
        \end{proof}
