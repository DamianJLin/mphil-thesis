...
Construction \ref{cons:lie-algebra-weight-system} yields a way of extracting some information from \(\mathcal{A}\) by plugging in a metric Lie algebra -- doing so constructs some quotient of \(\mathcal{A}\). This naturally begs the question whether all of the information in \(\mathcal{A}\) can be extracted by metric Lie algebras in this way.
...
\begin{definitions}
	...
	\end{enumerate}
\end{definitions}
The hexagon relations have look like they only have three terms instead of the six one might expect from the name. This is because we have omited the reassociation natural isomorphisms that we can assume are identites by Remark \ref{rem:coherence-for-monoidal-categories}.

If a monoidal category \(\mathcal{C}\) is additionally additive, we can define Lie algebra objects internal to \(\mathcal{C}\).

\begin{definitions}
	\begin{enumerate}
		\item A \textbf{Lie algebra object} in an additive symmetric tensor category \(\mathcal{C}\) is an object \(L\) equipped with a bracket morphism \(\beta : \mathcal{C} \otimes \mathcal{C} \to \mathcal{C}\) 
        such that
...
\begin{theorem}[\cite{vassiliev-knot-invariants-and-lie-s-algebras}]
	...
	Let \(\mathcal{C}\) be a rigid, additive, symmetric monoidal category, \(L\) a metric Lie algebra in \(\mathcal{C}\), and \(\eta\) a dualisable representation.
...
The original motivation for the work of Vogel \cite{algebraic-structures-on-modules-of-diagrams-preprint, algebraic-structures-on-modules-of-diagrams} (and it can be seen more explicity in \cite{vassiliev-theory-and-the-universal-lie-algebra}) was to construct a universal object generalising all simple Lie algebras. Such an object would parametrise the relations in tensor powers of the adjoint representation of all simple Lie algebras, and its weight systems would span \(\mathcal{W}\). For example, the relation for \(W_{\mathfrak{sl}_{2}}\) given in Example \ref{ex:sl2-cv-relation} is entirely internal to the Lie algebra; it doesn't involve projecting to the universal enveloping algebra or choosing a representation (since the adjoint representation is natural).

There seems to be a recent renewed interest in Vogel's methods \cite{construction-of-lie-algebra-weight-system-kernel-via-vogel-algebra, on-refined-vogels-universality, can-yang-baxter-imply-lie-algebra}. In-particular, present implicitly in \cite{algebraic-structures-on-modules-of-diagrams} is a way to determine local relations internal to any simple Lie algebra in terms of two independent so-called Vogel parameters, and some complicated `marked' elements of \(\mathcal{A}\) defined recursively. These were written out explicitly in the preprint \cite[Appendix]{construction-of-lie-algebra-weight-system-kernel-via-vogel-algebra}. The use of different normalisations of the Vogel parameters between this and \cite{algebraic-structures-on-modules-of-diagrams} makes computing explicit relations tedious. Furthermore, in Vogel's original work, the relations are general enough to hold for any Lie superalgebra. However when restricting to Lie algebras, the diagrammatic formulae can be simplified.
...
\begin{lemma}
	...
	\]
\end{lemma}
This is proven in \cite[Lem. 6.15]{introduction-to-vassiliev-invariants}. It would be interesting to have a purely categotical proof that doesn't refer to any specific representation, and uses the fact that Lie algebras are Lie algebra objects in the category \(\mathbf{Vect}\).

Simplifying Vogel's relation via these lemmas , gives the following relation. (We use the metric in which \(\lambda = 1/4\), i.e. our metric is \(4\) times \(\langle\cdot, \cdot\rangle_{K}\), the Killing form.)
...
This relation is true for any diagram into which each term is inserted. In-particular, it remains true after rotating all terms a quarter rotation clockwise. However this symmetry is not obvious in the formula. This suggests that perhaps the relation is a consequence of a similar, simpler relation.
...
The dimension of \(\mathfrak{g}_{2}\) is \(14\), and tensor contraction with sagemath is computable on a laptop in a few seconds. One the other hand, \(\operatorname{dim} \mathfrak{f}_{4} = 52\), and calculation takes several hours on a server with two Intel Xeon E5-2667v3 CPUs. The main bottleneck is tensor contraction, and indeed for \(\mathfrak{e}_{6}\) (\(\operatorname{dim} \mathfrak{e}_{6} = 78\)), the computation cannot complete even in several days. There is some hope that this can be done by using more efficient tensor contraction algorithms than are offered by sagemath, but this has not yet been explored.
...
\begin{example}
	\label{ex:lie-operad}
	Let \(\Gamma\) denote the free operad functor (for details we refer the reader to \cite{operads-and-props}). Let
	\[
		\alpha
		=
		\def\svgscale{0.2}
...
\begin{definition}
	The \textbf{prop for casimir algebras over the cyclic operad} \(\mathcal{O}^{\text{cyc}}\), denoted \(\mathbf{E}_{c}(\mathcal{O}^{\text{cyc}})\) is the prop generated by the prop \(\mathbf{E}(\mathcal{O})\) along with the element \(c \in \mathbf{E}_{c}(\mathcal{O}^{\text{cyc}})(\mathbf{0}, \mathbf{2})\) such that:
	\begin{itemize}
...
\begin{example}
	Let \(\mathfrak{g}\) be a Lie algebra (in \(\mathbf{Vect}\)). We examine the relation in the external enveloping algebra \(U(\mathcal{O}_{\text{Lie}}, \mathfrak{g})\) coming from making the diagram of Definition \ref{def:external-enveloping-algebra} commute in the case \(n = 1, m_{1} = 2\) on the object \([\cdot,\cdot] \otimes [\cdot,\cdot] \otimes \xi\). Let us write \(\xi = \xi(1) = \xi_{1} \otimes \xi_{2}\).

	Applying the Lie action map first gives
	\[[\cdot,\cdot] \otimes_{S_{n}} [\xi_{1}, \xi_{2}],\]
	whereas applying the operadic composition first gives
	\[\left( [\cdot,\cdot] \circ_{2} [\cdot, \cdot] \right) \otimes_{S_{n}} (\xi_{1} \otimes \xi_{2}).\]
	In order to easily work the the relations in the operad \(\mathcal{O}_{\text{Lie}}\), let us represent \([\cdot,\cdot] \circ_{2} [\cdot, \cdot] \in \mathcal{O}_{\text{Lie}}(3)\) as
	\[
		[\cdot,\cdot] \circ_{2} [\cdot, \cdot]
		=
		\def\svgscale{0.2}
		\adjustbox{valign=c}{\input{graphics/external_enveloping_algebra_commutator.pdf_tex}}
		.
	\]
	by the relations in from Example \ref{ex:lie-operad}, we have
	\[
		\def\svgscale{0.2}
		\adjustbox{valign=c}{\input{graphics/external_enveloping_algebra_commutator.pdf_tex}}
		=
		...
		.
	\]
	But, in view of the multiplication in \(T(\mathcal{O}, A)\) from Definition \ref{def:external-tensor-algebra}, this is equal to
	\[
		([\cdot,\cdot] \otimes_{S_{n}} \xi_{1}) \cdot ([\cdot, \cdot] \otimes_{S_{n}} \xi_{2})
		-
		([\cdot,\cdot] \otimes_{S_{n}} \xi_{2}) \cdot ([\cdot, \cdot] \otimes_{S_{n}} \xi_{1})
		.
	\]
	In total
	\[
		...
		([\cdot,\cdot] \otimes_{S_{n}} \xi_{2}) \cdot ([\cdot, \cdot] \otimes_{S_{n}} \xi_{1})
	\]
	and this starts to look like a universal enveloping algebra.
\end{example}

\begin{theorem}[Hinnich-Vaintrob]
	\(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c})\) is isomorphic as a bialgebra to \(\mathcal{A}\).
\end{theorem}
