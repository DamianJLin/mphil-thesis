...
Quite surprisingly, Conjecture \ref{conj:lie-algebra-weight-systems-span} is false. This is hard to believe, as the \ref{eq:STU} relation and the relation \([x, y] = x y - y x\) in \(\mathcal{U}(\mathfrak{g})\) look like they encode exactly the same thing. First, we give a non-rigorous reframing of the construction of Bar-Natan following \cite{algebraic-structures-on-modules-of-diagrams, vassiliev-knot-invariants-and-lie-s-algebras} to convince the reader that actually \([x, y] = xy - yx\) is a specialisation of \ref{eq:STU}. Later in the section, we will give a rigorous construction of \cite{cyclic-operads-and-the-algebra-of-chord-diagrams} which provides a moral version of the statement in Remark \ref{rem:chord-diagram-universal-construction}.

Effectively: We can make the internal part of \(\mathcal{A}\) into a monoidal category. Then the construction is a representation of \(\mathcal{A}\) in the monoidal category of tensor powers of some Lie algebra \(L\). And then, it has to obey the universal enveloping algebra relations too. Effectively, it comes down to a representation of \ref{eq:STU} where the changing an \(xy\) to a \(yx\) is some representation of the symmetric group on some vector space. In the case of Lie-algebras, the representation is the identity. But it doesn't have to be.
