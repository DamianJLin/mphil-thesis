...
\begin{definition}
	...
	\[(a \circ_{1} b) \tau = (b\tau) \circ_{n} (a\tau)\]
	for any \(a \in \mathcal{O}(m)\), \(b \in \mathcal{O}(n)\). This has the following graphical interpretation.
	\[
		\tau \left(
		\;
		\def\svgscale{0.3}
	...
	We denote an arbitrary cyclic operad \(\mathcal{O}_{\tau}\) to reflect that whether an operad is cyclic depends both on \(\mathcal{O}\) and the action of the symmetry \(\tau\) of \(\mathcal{C}\) on \(\mathcal{O}\).
\end{definition}
...
\begin{definition}
	For a cyclic operad \(\mathcal{O}_{\tau}\) in category \(\mathcal{C}\), a \textbf{metric algebra} over \(\mathcal{O}_{\tau}\) is a choice of \(A \in \mathcal{C}\) with together with a symmetric morphism \(b: A \otimes A \to k\), such that \(b\) is:
	\begin{itemize}
		\item  \textbf{invertible}: there exists a morphism \(c: k \to A \otimes A\) such that the composition
			\[A \overset{c \otimes \id}{\longrightarrow} A \otimes A \otimes A \overset{id \otimes b}{\longrightarrow} A\]
			is the identity.
			\begin{shaded}
			\item \(\mathcal{O}\)-\textbf{invariant}: the composition
				\[\mathcal{O}(n) \otimes A^{\otimes n + 1} \longrightarrow A \otimes A \overset{b}{\longrightarrow} k\]
				is \(S_{n + 1}\)-invariant.

				(REALLY? Surely it just has to agree with the \(S_{n}\)-action.)
			\end{shaded}
	\end{itemize}
\end{definition}

The conditions above can be more naturally phrased in terms of props.
