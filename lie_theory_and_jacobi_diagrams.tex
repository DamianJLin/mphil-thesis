...
\lettrine{\libertineInitialGlyph{T}}{he} fundamental theorem of Vassiliev invariants states that the bialgebra of Vassiliev invariants can be broken up into ``nice'' combinatorial weight systems. So to understand \(\mathcal{V}\) it suffices to understand \(\mathcal{W}\) (or equivalently its dual \(\mathcal{A}\)).

As we will see in this chapter, the structures of \(\mathcal{W}\) and \(\mathcal{A}\) are closely related to Lie theory: from any metric Lie algebra, there is construction that associates to it a weight system. This by itself is not enough to produce all weight systems --- to do so needs the construction must be extended to a generalised notion of metric Lie algebras. The metric Lie algebras are generalised to metric Lie algebra objects in an arbitrary symmetric monoidal category to account for the possibility of nontrivial symmetry natural isomorphisms in the symmetric monoidal category, and thereby every weight system can be obtainted.

An operadic framework is introduced to prove this fact, and metric Lie algebra objects are replaced by the restricted notion of Casimir Lie algebra objects for technical reasons.
...
The result is standard --- see for example Chapter 5.2 of \cite{introduction-to-vassiliev-invariants} for a proof. A corollary is the following result, which will be important later.
...
\begin{proof}
	...
	We can split \(\Theta\) up based on which connected component the marked vertex now connects to. For connected components other than the connected component of the marked vertex, apply a vertical version of the generalised \ref{eq:IHX} relation (Proposition \ref{prop:generalised-ihx}), to the terms in which the marked vertex connects to that component. This is an \ref{eq:IHX} in which the grey box in Proposition \ref{prop:generalised-ihx} is around the whole connected component, except for where the univalent vertices connect to the skeleton.
	As such, this is the case of the generalised \ref{eq:IHX} in which \(n = 0\):
	\[
		\sum \includegraphics[width=0.25\textwidth, valign=c]{graphics/long_jacobi_diagram_example_diff_cc.pdf}
		\quad
		=
		\quad
		0.
	...
	This leaves only the terms where the marked vertex connects back to its own connected component. The sum of such terms also vanishes: by a generalised \ref{eq:IHX} relation, we have
	\[
...
\begin{proposition}
	\label{prop:jacobi-diagram-coproduct}
...
\begin{corollary}
	\label{cor:primitives-equals-connected-jacobi-diagrams}
...
Similar diagrammatic relations to \ref{eq:STU}, \ref{eq:AS} and \ref{eq:IHX} satisfied in \(\mathcal{A}\) appear also in the context of a graphical notation for multilinear maps, a fact which can be exploited to probe \(\mathcal{A}\). Before seeing how, let us review this graphical notation following \cite{wheeling-a-diagrammatic-analogue-of-the-duflo-isomorphism, on-the-rozansky-witten-weight-systems}. The diagrammatic calculus is well-known but it goes by many names including string diagram calculus, Penrose calculus, tensor calculus, diagrammatic calculus. We call them string diagrams.

% NOTE: This is all true for tensors in the category FinDimVect, not arbitrary categories. fix?
\begin{definition}
	\label{def:tensor-as-multilinear-map}
	A \textbf{tensor} is a multilinear map \(X_{1} \times X_{2} \times \cdots \times X_{n} \to Y_{1} \times Y_{2} \times \cdots \times Y_{m}\)
	for some vector spaces \(X_{1}, \dots, X_{n}\) and \(Y_{1}, \dots, Y_{m}\).

	The data \(n, m \in \mathbb{N}\) and the spaces \(X_{i}\) and \(Y_{j}\) for \(0 \leq i \leq n\) and \(0 \leq j \leq m\) is known as the \textbf{signature} of the tensor.
\end{definition}

\begin{proposition}
	\label{prop:tensor-multilinear-map-canonical-isomorphism}
	The vector space of tensors with a given signature
	\[X_{1} \times \cdots \times X_{n} \longrightarrow Y_{1} \times \cdots \times Y_{m}\]
	where each \(X_{i}\) and \(Y_{j}\) are finite-dimensional vector spaces is naturally isomorphic as a vector space to the space \(X_{1}^{\ast} \otimes \cdots \otimes X_{n}^{\ast} \otimes Y_{1} \otimes \cdots \otimes Y_{m}\).
\end{proposition}

\begin{proof}
	Fix a vector space \(V\). The natural isomorphism is a composition of the following three natural isomorphisms.

	Firstly, the universal property of the tensor product gives a natural isomorphism between the space of multilinear maps of the form \(X_{1} \times \cdots \times X_{n} \to V\), and the space of linear maps of the form \(X_{1} \otimes \cdots \otimes X_{n} \to V\).

	Secondly, from the tensor-hom adjunction we get a natural isomorphism between maps of the form \(W \to V\) and the space \(V \otimes W^{\ast}\).

	Finally, for finite-dimensional vector spaces there is a natural isomorphism \((V_{1} \otimes \cdots \otimes V_{m})^{\ast}\) to \(V_{1}^{\ast} \otimes \cdots \otimes V_{m}^{\ast}\).

	Composing the first two natural isomorphisms with \(V = Y_{1} \otimes \cdots \otimes Y_{m}\), then the final with \(V_{i} = Y_{i}\) completes the proof.
\end{proof}

\begin{definition}
	\label{def:string-diagram}
	A \textbf{string diagram} is an equivalence class of labelled progressive planar graphs under progressive deformation.
\end{definition}

The details of the definition are beyond the scope of this thesis, however the interested reader is invited to consult \cite{the-geometry-of-tensor-calculus-one}. For our purposes, string diagrams constitute a graphical calculus for the operations of partial composition and tensor product in monoidal categories. A tensor of signature \(X_{1}^{\ast} \otimes \cdots \otimes X_{n}^{\ast} \otimes Y_{1} \otimes \cdots \otimes Y_{m}^{\ast}\) is represented by a string diagram consisting of a vertex, labelled by the tensor, with \(n\) incoming edges, labelled by the domain spaces and \(m\) outgoing edges, labelled by the codomain spaces. We read these diagrams top-to-bottom, so domain edges come from above, and codomain arrows from below.

% TODO: a little sus...
The operation of partial composition of tensors corresponds to composing two string diagrams via vertically concatenating domain and codomain edge whose labellings agree. The tensor product of two tensors (as multilinear functions) corresponds to the string diagram formed by horizontal concatenation of two string diagrams. The use of string diagrams is that progressive deformation of string diagrams correspond to equivalent ways of performing iterated partial compositions and tensor products.

For example, the bracket in a Lie algebra \(\mathfrak{g}\) is tensor \([\cdot,\cdot]\) of signature \(\mathfrak{g}^{\ast} \otimes \mathfrak{g}^{\ast} \otimes \mathfrak{g}\) expressed via the string diagram
\[
	\def\svgscale{0.3}
	\adjustbox{valign=c}{\input{graphics/bracket_string_diagram_labelled.pdf_tex}}
	\qquad
	\text{or}
	\qquad
	\def\svgscale{0.3}
	\adjustbox{valign=c}{\input{graphics/bracket_string_diagram.pdf_tex}}
	\qquad
\]
where we drop the vertex labels, which can be read from context.
...
\[
	...
	\text{and}
\]
where here the new type of vertex on the left hand side of signature \(\mathfrak{g} \otimes \mathfrak{g} \to \mathfrak{g} \otimes \mathfrak{g}\) represents the swap map \(\sigma: x \otimes y \mapsto y \otimes x\). Similarly, the Jacobi relation \([[x, y], z] + [[y, z], x] + [[z, x], y] = 0\) becomes
\[
...
\begin{definition}
	\label{def:metric-lie-algebra}
	A Lie algebra is \textbf{metric} if it has an ad-invariant, nondegenerate, bilinear form. That is a bilinear form \(\langle \cdot , \cdot \rangle\) such that for all \(x, y, z \in \mathfrak{g}\), \(\langle [x, y], z\rangle = \langle x, [y, z]\rangle\). If \(\mathfrak{g}\) is metric, we call its bilinear form a metric.
\end{definition}

Now suppose \(\mathfrak{g}\) is a metric Lie algebra. Its metric \(\langle \cdot, \cdot \rangle \in \mathfrak{g}^{\ast} \otimes \mathfrak{g}^{\ast}\) is nondegenerate so it always has an adjoint element \(c \in \mathfrak{g} \otimes \mathfrak{g}\). This element is known as the Casimir element associated to \(\mathfrak{g}\) (or simply the Casimir). These two tensors, the metric and the Casimir can be diagrammatically represented as additional bivalent vertices
\[
	...
	\qquad \text{or dropping the vertices} \qquad
...
Moreover, the invariance of the metric,  \(\langle [x, y], z \rangle = \langle [y, z], x \rangle\) can be represented graphically as cyclic invariance of the tensor \(\langle [x, y], z\rangle\):
\[
...
\]
Since the Casimir element is adjoint to the metric, the invariance of the metric should also be reflected in a property of the Casimir.

\begin{proposition}
	\label{prop:casimir-element-invariance}
	Let \(c = \sum_{i} c_{i1} \otimes c_{i2}\) be the Casimir element of a metric Lie algebra. Then for \(x \in \mathfrak{g}\), it obeys the relation
	\[\sum_{i} c_{i1} \otimes [c_{i2}, x] = \sum_{i} [x, c_{i1}] \otimes c_{i2}\]
	...
	\]
\end{proposition}
% NOTE: This should really be done by string diagrams but, the reviewer asked for a proof.
\begin{proof}
	Applying the invariance of the metric \(\langle [x, y], z \rangle = \langle x, [y, z] \rangle\) to the expression
	\(\sum_{i, j} \langle [c_{i2}, x], c_{j1}\rangle c_{i1} \otimes c_{j2}\)
	gives
	\[
		\sum_{i, j} \langle [c_{i2}, x], c_{j1}\rangle c_{i1} \otimes c_{j2}
		=
		\sum_{i, j} \langle c_{i2}, [x, c_{j1}] \rangle c_{i1} \otimes c_{j2}.
	\]
	Since the metric and the Casimir elements are adjoint, for all \(x \in \mathfrak{g}\) we have \(\sum_{i} c_{i1} \langle c_{i2}, x \rangle = x = \sum_{i} c_{i2} \langle x, c_{i1} \rangle\). Applying this to the equality above yields
	\[\sum_{i} c_{i1} \otimes [c_{i2}, x] = \sum_{j} [x, c_{j1}] \otimes c_{j2}\]
	which is the desired result after a change of index.
\end{proof}

From the diagrammatic interpretations of the antisymmetry and Jacobi relations above, we see that the internal structure of Jacobi diagrams (internal meaning not near the skeleton) is reflected in a metric Lie algebra. Let us try to find a similar reflection of the structure near the skeleton. A representation \(\rho\) of \(\mathfrak{g}\) on a finite-dimensional vector space \(V\) can be written as a tensor \(\rho \in \mathfrak{g}^{\ast} \otimes V^{\ast} \otimes V\). This takes a new kind of input and output, namely a \(v \in V\) which we denote by a thicker line, like the skeleton of a Jacobi diagram.
\[
	\includegraphics[width=0.06\textwidth, valign=c]{graphics/representation_vertex_before_dualisable.pdf}
\]
...
\]
Recall that once again the four-valent vertex in this string diagram corressponds to the swap map \(\sigma: \mathfrak{g} \otimes \mathfrak{g} \to \mathfrak{g} \otimes \mathfrak{g}; x \otimes y \mapsto y \otimes x\).
...
\begin{example}
	...
	Adjoint to the metric is the Casimir element
	\[c = \frac{1}{2} h^{\ast} \otimes h^{\ast} + e^{\ast} \otimes f^{\ast} + f^{\ast} \otimes e^{\ast}.\]
	...
	\[	
		J = \includegraphics[width=0.1\textwidth, valign=c]{graphics/jacobi_bivalent_bubble.pdf}.
	\]
...
Conjecture \ref{conj:lie-algebra-weight-systems-span} is false and the \ref{eq:STU} relation describes more than just the relation \(\rho([x, y]) = \rho(x)\rho(y) - \rho(y)\rho(x)\) for a representation \(\rho\) of a metric Lie algebra. This should be surprising since the string diagrammatic version of \(\rho([x, y]) = \rho(x)\rho(y) - \rho(y)\rho(x)\) (just above Construction \ref{cons:lie-algebra-weight-system}) looks identical to the \ref{eq:STU} relation.

The hidden assumption of \(\rho([x, y]) = \rho(x)\rho(y) - \rho(y)\rho(x)\) is that the isomorphism used to rearrange the tensor factors is the swap map \(\sigma: x \otimes y \mapsto y \otimes x\) which needn't be the case.

In fact, the construction of metric Lie algebra weight systems seen in \ref{cons:lie-algebra-weight-system} is just one example of a more general construction introduced by Vogel and Vaintrob. Lie algebras, are Lie algebra objects in the symmetric monoidal category of vector spaces, which happens to be a strict symmetric monoidal category, meaning the symmetry natural isomorphism is the swap map \(\sigma\). However the more general construction also works for non-strict symmetric monoidal categories. To name an example that we will come back to in detail in Example \ref{ex:symmetric-monoidal-categories}~\ref{ex:cat-super-vector-spaces}, in the category of super vector spaces, the Lie algebra objects are the Lie superalgebras, and the symmetry isomorphism is not the swap map.

\begin{remark}
	The more general construction was originally developed by Vaintrob as applicable to a class of objects called ``Lie \(S\)-algebras''. The more modern and categorical approach we choose to follow in this section of Lie algebra objects in arbitrary symmetric monoidal categories is due to \cite{on-the-rozansky-witten-weight-systems, rozansky-witten-theory}
\end{remark}
...
\begin{definition}
	...
	\[
		\otimes \circ (\otimes \times \operatorname{id}) \longrightarrow \otimes \circ (\operatorname{id} \times \otimes) ,\quad k \otimes \_ \longrightarrow \id \quad \text{ and }\ \quad \_ \otimes k \longrightarrow \operatorname{id}
	\]
...
\begin{definitions}
	\begin{enumerate}
		...
		\item A \textbf{symmetric monoidal category} is a monoidal category \(\mathcal{C}\) equipped with a \textbf{symmetry natural isomorphism}
			...
			\[\tau_{A, B \otimes C} = (\id_{B} \mathbin{\otimes} \tau_{A, C}) \circ (\tau_{A, B} \mathbin{\otimes} \id_{C})\]
			for every triple of objects \(A\), and \(B\) in \(\mathcal{C}\). 
	\end{enumerate}
\end{definitions}
One might notice that the hexagon relation involves only three terms, rather than the six that the name suggests. This is because we have omited the reassociation natural isomorphisms that we assume are identities by Remark \ref{rem:coherence-for-monoidal-categories}.

\begin{examples}
	\label{ex:symmetric-monoidal-categories}
	\begin{enumerate}
		\item
			\label{ex:cat-vector-space}
			Let \(\mathbf{Vect}\) be the category of vector spaces and linear transformations. The symmetry natural isomorphism
			\[
				\begin{array}{rccc}
					\tau:	& X \times Y		&\longrightarrow	& Y \times X\\
						& x \otimes y		&\longmapsto		& y \otimes x.
				\end{array}
			\]
			gives \(\mathbf{Vect}\) the structure of a symmetric monoidal category.
		\item
			\label{ex:cat-super-vector-spaces}
			A \textbf{super vector space} is a \(\mathbb{Z}/2\mathbb{Z}\)-graded vector space \(V\) with the decomposition
			\[V = V_{0} \oplus V_{1}.\]
			The degree, \(|x|\) of a homogeneous element \(x \in V = V_{0} \oplus V_{1}\) is \(|x| = i \in \mathbb{Z}/2\mathbb{Z}\) where \(x \in V_{i}\).

			Let \(\mathbf{sVect}\) denote the category of super vector spaces and degree preserving linear transformations. Defining the symmetry natural isomorphism on homogeneous elements \(x, y \in V\) as
			\[
				\begin{array}{rccc}
					\tau:	& X \times Y		&\longrightarrow	& Y \times X\\
				...
				\end{array}
			\]
			gives \(\mathbf{sVect}\) the structure of a symmetric monoidal category.
	\end{enumerate}
\end{examples}

If (like in our examples) the monoidal category \(\mathcal{C}\) is additive and the functor \(\otimes: \mathcal{C} \times \mathcal{C} \to \mathcal{C}\) is bilinear, then Lie algebra objects internal to \(\mathcal{C}\) can be defined. These generalise Lie algebras in that they satisfy analogues of the Jacobi and antisymmetry identities involving the symmetry natural isomorphism \(\tau\). Of course, in the case \(\mathcal{C} = \mathbf{Vect}\), \(\tau\) is the trivial swap isomorphism, so the Lie algebra objects satisfy exactly the Jacobi and antisymmetry relations and are exactly the Lie algebras.

\begin{definitions}
	\label{def:internal-to-additive-monoidal-category}
	\begin{enumerate}
		\item \label{subdef:lie-algebra-object}
			A \textbf{Lie algebra object} in \(\mathcal{C}\) is an object \(L\) equipped with a bracket morphism \(\beta : \mathcal{C} \otimes \mathcal{C} \to \mathcal{C}\) such that
			...
			\[
				...
				0.
			\]
			Here the trivalent vertex corresponds to the bracket morphism \(\beta\).
		\item A (right) \textbf{module \(\rho\) over a Lie algebra object \(L\) in \(\mathcal{C}\)} is an object \(M\) with an action morphism \(\rho: M \otimes L \to M\), diagramatically denoted
			\[
				\includegraphics[width=0.06\textwidth, valign=c]{graphics/representation_vertex_before_dualisable.pdf}
			\]
			such that
			\[\rho \circ (\id \otimes \beta) = \rho \circ (\rho \otimes \id) - \rho \circ (\rho \otimes \id ) \circ (\id \otimes \tau).\]
			Here the thick line denotes \(M\) and this type of trivalent vertex denotes \(\rho\). As a string diagram the equation becomes
			\[
				\def\svgscale{0.31}
			...
			\[
				...
				-
				\;
				\includegraphics[width=0.06\textwidth, valign=c]{graphics/representation_vertex_before_dualisable_flipped.pdf}
				\;
				.
			\]
		\item A \textbf{metric Lie algebra object} \(L\) in \(\mathcal{C}\) is a Lie algebra object in \(\mathcal{C}\), further equipped with a metric morphism \(m: L \otimes L \to k\) and Casimir morphism \(c: k \to L \otimes L\) which abstract the metric and Casimir of Lie algebras in \(\mathbf{Vect}\) 
		by satisfying
			\[(\id \otimes c) \circ (m \otimes \id) = \id = (c \otimes \id) \circ (\id \otimes m)\]
			and
			\[m = \tau \circ m \quad \text{and} \quad c = c \circ \tau.\]
			Diagramatically, these morphisms are
			\[
				\includegraphics[width=0.06\textwidth, valign=c]{graphics/string_diagram_metric_no_dot.pdf}
				\qquad \text{and} \qquad
				\includegraphics[width=0.06\textwidth, valign=c]{graphics/string_diagram_casimir_no_dot.pdf}\ .
			\]
			(like before these morphisms allow us to drop arrows on the thin lines). The former pair of relations is diagramatically
			\[
				...
				\quad
			\]
			and the latter pair is
			\[
				\includegraphics[width=0.06\textwidth, valign=c]{graphics/string_diagram_metric_no_dot.pdf}
				\quad
				=
				\quad
				\includegraphics[width=0.06\textwidth, valign=c]{graphics/string_diagram_metric_no_dot_is_symmetric.pdf}
				\qquad \text{and} \qquad
				\includegraphics[width=0.06\textwidth, valign=c]{graphics/string_diagram_casimir_no_dot.pdf}
				\quad
				=
				\quad
				\includegraphics[width=0.06\textwidth, valign=c]{graphics/string_diagram_casimir_no_dot_is_symmetric.pdf}\ .
			\]
		\item
			\label{def:rigid-category-and-object}
			A symmetric monoidal category \(\mathcal{C}\) is \textbf{rigid} if for every object \(A\) there exists an object \(A^{\vee}\) and a pair of morphisms \(\iota: A^{\vee} \otimes A \to k\) and \(\varepsilon: k \to A \otimes A^{\vee}\) --- respectively called the \textbf{evaluation} and \textbf{coevaluation} --- such that
			\[(\id_{A} \otimes \varepsilon) \circ (\iota \otimes \id_{A}) = \id_{A}\]
			and
			\[(\varepsilon \otimes \id_{A^{\vee}}) \circ (\id_{A^{\vee}} \otimes \iota) = \id_{A^{\vee}}\]
...
We will give concrete examples of Lie algebra objects in different symmetric monoidal categories later. For now, let's show that this data can be used to construct weight systems, generalising Construction \ref{cons:lie-algebra-weight-system}.

\begin{theorem}[\cite{vassiliev-knot-invariants-and-lie-s-algebras}]
	Let \(\mathcal{C}\) be a rigid, additive, symmetric monoidal category, \(L\) a metric Lie algebra in \(\mathcal{C}\), and \(M\) a rigid module (a rigid object in the monoidal category of right modules over \(L\)).
...
Recall, the difference between this construction and Construction \ref{cons:lie-algebra-weight-system} is the treatment of the symmetry natural isomorphism \(\tau\) which tells us what isomorphism to use when rearranging the tensor factors. The most obvious isomorphism would be the identity (as it was in the original construction) corresponding to when \(\mathcal{C}\) is a strict symmetric (strict) monoidal category. However not every symmetric monoidal category is equivalent to a strict symmetric monoidal category, and algebra objects with non-trivial symmetry isomorphisms are evidently necessary to pick up all the structure in \(\mathcal{A}\).

\begin{example}
	If we take \(\mathcal{C} = \mathbf{sVect}\), the symmetric monoidal category of super vector spaces, then the Lie algebra objects are exactly the Lie superalgebras which we now define.

	A \textbf{Lie superalgebra} \(\mathfrak{g}\) is a vector space with a \(\mathbb{Z} / 2\mathbb{Z}\) grading, equipped with a bracket \([\cdot,\cdot]: \mathfrak{g} \otimes \mathfrak{g} \to \mathfrak{g}\) satisfying two axioms. The \textbf{super symmetry} axiom is that for \(x, y\) homogeneous elements of \(\mathfrak{g}_{i}\) and \(\mathfrak{g}_{j}\) respectively,
	\[[x, y] = - (-1)^{ij}[y, x].\]
	The \textbf{super Jacobi identity}, that for homogeneous elements \(x, y\) and \(z\) in \(\mathfrak{g}_{i}, \mathfrak{g}_{j}\) and \(\mathfrak{g}_{k}\),
	\[(-1)^{ik}[x, [y, z]] + (-1)^{ji}[y, [z, x]] + (-1)^{kj}[z, [x, y]] = 0.\]
	Note that these identities are simply the axioms in Definition \ref{def:internal-to-additive-monoidal-category} \ref{subdef:lie-algebra-object}, specialised to the category \(\mathcal{C} = \mathbf{sVect}\).
...
\begin{theorem}
	There are primitive Jacobi diagrams of order 17 \cite{algebraic-structures-on-modules-of-diagrams-preprint} and 15 \cite{on-vassiliev-invariants-not-coming-from-semisimple-lie-algebras} on which all Lie algebra weight systems vanish.
\end{theorem}
...
As such, we see that the Lie algebra objects in the category \(\mathcal{C} = \mathbf{Vect}\) and even the more general category \(\mathcal{C} = \mathbf{sVect}\) do not suffice to pick up all the structure in \(\mathcal{A}\).

\begin{corollary}
	The set of Lie (super)algebra weight systems does not span \(\mathcal{W}\).
\end{corollary}

In general, it is still unknown what exact level of generality one needs (what type of symmetric monoidal categories need to be considered) in order to generate all weight systems, leaving Question \ref{ques:vogel-universal-lie-algebra} an open question. We hereby provide a succinct review of the current state of the literature on the subject.

Roberts and Willerton in \cite{rozansky-witten-theory} and \cite{on-the-rozansky-witten-weight-systems} examine weight systems constructed from Lie algebra objects in the derived category of chain complexes. Such weight systems are candidates for being able to detect knot orientation, which Lie algebra weight systems cannot \cite{on-the-vassiliev-knot-invariants}. However, computing these weight systems is difficult, and to our knowledge, no computations exist in the literature.
...
\[
	...
	\end{array}
\]
in \(\mathcal{W}_{\mathfrak{g}_{2}}\). By some well-known identites internal to Jacobi diagrams, this relation can be simplified as follows.
\begin{lemma}
...
\begin{theorem}
	\label{thm:bookshelf-g2-formula}
...
\begin{proof}
	...
	\begin{equation}
		\tag{\ref{thm:bookshelf-g2-formula}a}
...
Finally, we note that for \(\mathfrak{sl}_{4}\) there is no similar linear relation between the same diagrams.

\begin{theorem}
	\label{thm:no-sl4-unboxing-relation}
...
As a result, the methods applied to \(\mathfrak{g}_{2}\) and \(\mathfrak{f}_{4}\) cannot be used to produce a similar quadraticity bound for the values of the bookshelf diagrams under the \(\mathfrak{sl}_{4}\) weight system.

\section{A universal casimir Lie algebra object}
\label{sec:a-universal-casimir-lie-algebra-object}

We return to the universal Lie algebra of Vogel which was discussed in Section \ref{sec:non-lie-algebraic-weight-systems}. For a long period, Vogel endevoured \cite{algebraic-structures-on-modules-of-diagrams-preprint, algebraic-structures-on-modules-of-diagrams, vassiliev-theory-and-the-universal-lie-algebra} to construct this univeral Lie algebra, however attempts at its construction have so far failed \cite{can-yang-baxter-imply-lie-algebra, on-refined-vogels-universality, construction-of-lie-algebra-weight-system-kernel-via-vogel-algebra}.

There is, however, a theoretical construction of Hinnich--Vaintrob \cite{cyclic-operads-and-the-algebra-of-chord-diagrams}: they construct a Lie algebra object in a symmetric monoidal category with a weight system which is isomorphic to \(\mathcal{A}\) as a bialgebra. This guarantees that every Vassiliev invariant can be constructed from applying Construction \ref{cons:lie-algebra-weight-system} to some Lie algebra object in some symmetric monoidal category. However, it doesn't give any hints as to which symmetric monoidal category one needs to work in to construct a particular Vassiliev invariant. In this sense it is useful theoretically, but not computationally.

Furthermore, the this bialgebra is constructed as a generalised notion of a universal enveloping algebra, applied to a Lie algebra object is some symmetric monoidal category, giving  credence to the statement hinted at by Construction \ref{cons:lie-algebra-weight-system} that ``the algebra \(\mathcal{A}\) behaves like the universal enveloping algebra of a Lie algebra''.

In this section we review the Hinich--Vaintrob construction, and in Chapter \ref{ch:welded-knots-isometry-lie-algebras-and-arrow-diagrams}, we will generalise it from Vassiliev invariants of classical knots to those of welded long knots.

A concept that is essential to the construction of Hinich--Vaintrob is that of an operad, which captures abstractly the general behaviour of associative operations and their iterated composition. Indeed, when operations on a set are composed by taking as one of their inputs the output of another operation, these compositions always obey certain rules. If the set is for example a vector space, the operations too form a vector space, and there is an action of the symmetric group \(S_{n}\) coming from the permutation of the inputs of the operation. An operad captures this concept of operations on some object of a category abstractly. 

\begin{warning}
	For all of the below, we assume the monoidal categories are \(k\)-linear, \(k\) a field of characteristic zero and the tensor product is bilinear (also known as tensor categories).
\end{warning}

\begin{definition}
	\label{def:operad-over-set}
	An \textbf{operad} \(\mathcal{O}\) in the symmetric monoidal category \(\mathbf{Set}\) consists of:
	\begin{itemize}
		\item a collection of sets, \(\{\mathcal{O}(n)\}_{n \in \mathbb{N}}\), with \(\mathcal{O}(n)\) known as the set of \textbf{\(n\)-ary operations},
		\item an element \(\id \in \mathcal{O}(1)\),
		\item an action of the symmetric group \(S_{n}\) on each \(\mathcal{O}(n)\)
		\item a series of \textbf{composition functions}:
        for each \(n\) and \(k_{1}, k_{2}, \cdots, k_{n} \in \mathbb{N}\)
			\[
				\begin{array}{rccc}
					\circ_{n}:
	...
	\end{itemize}
		\begin{enumerate}
			...
			\item \textbf{Associativity of composition}. Let \(\theta \in \mathcal{O}(n)\), and \(\theta_{1}\in \mathcal{O}(k_{1})\), \(\theta_{2} \in \mathcal{O}(k_{2})\), up to \(\theta_{n} \in \mathcal{O}(k_{n})\), and finally let \(\theta_{1, 1} \in \mathcal{O}(k_{1, 1})\), \(\theta_{1, 2} \in \mathcal{O}(k_{1, 2})\) through to \(\theta_{k_{n}, 1} \in \mathcal{O}(k_{k_{n}, 1})\). The two ways of forming the composition of the three layers of operations: \(\theta_{i, j}\) into \(\theta_{i}\) into \(\theta\) are equal. That is,
			...
			\item That the action of the symmetric group agrees with its action in the symmetric monoidal category \(\mathbf{Set}\).
		\end{enumerate}
\end{definition}

Accordingly, operads can be considered over any symmetric monoidal category \(\mathcal{C}\) by replacing \(\mathbf{Set}\) with \(\mathcal{C}\) in Definition \ref{def:operad-over-set} and adjusting sets to be objects of \(\mathcal{C}\) and functions between sets to be morphisms between the corresponding objects.

The operations in an operad can be also be represented graphically. An operation of arity \(n\) is drawn as a ball with \(n\) input lines (coming from above) and a single output line.
	\[
		\def\svgscale{0.3}
		\input{graphics/operad_operation.pdf_tex}
	\]
Composition of operads can then be denoted graphically by connecting the inputs of an operation to outputs of other operations, to form a new compound operation of greater or equal arity.
	\[
		\def\svgscale{0.3}
		\input{graphics/operad_multiplication.pdf_tex}
	\]
This way, relations in an operad can be expressed graphically. As a first example, the associativity axiom in Definition \ref{def:operad-over-set} asserts that when doing doing two series of compositions between three layers of operations, there is no need for brackets that specify a vertical bracketing. That is, connecting the first layer to the second layer to create a compound operation, then connecting this to the final layer creates the same operation as first connecting the second and third layers, and then taking the composition with the first layer. It is because of this axiom that the diagrams are well-defined without needing a vertical bracketing.

% TODO: Make this precise and put in an appropriate environment and make Lie algebras an example of this.
% NOTE: The compatibility conditions here are the following:
% consider the type of diagram we are familiar with
% objects objects objects (of C)
% first layer of operads
% second layer of operad
% FIRST CONDITION: that plugging the objects in first to the first operations, then plugging in the resulting objects into the final poeration == plugging the objects into the product of the operations

\begin{definition}
	\label{def:algebra-over-operad}
	An \textbf{algebra over an operad \(\mathcal{O}\) in the symmetric monoidal category \(\mathcal{C}\)} is an object \(X\) of \(\mathcal{C}\) with a sequence of \textbf{operation evaluation maps}
	\[\mathcal{O}(n) \otimes X^{\otimes n} \longrightarrow X\]
	for all \(n \in \mathbb{N}\), such that
	\begin{itemize}
		\item the operation evaluation maps are compatible with operad composition,
		\item the operation evaluation maps are compatible with permutations in the symmetric monoidal category \(\mathcal{C}\),
		\item for \(\id \in \mathcal{O}(1)\), the operation evaluation map \(\id \otimes X \to X\) is the identity map on objects of \(\mathcal{C}\).
	\end{itemize}
	All of these conditions are written out explicitly in \cite[Ch. 3]{operads-and-props}.
\end{definition}

Essentially, an algebra over an operad in \(\mathcal{C}\) is an object in \(\mathcal{C}\) that comes equipped with operations whose successive compositions obey the rules defined by \(\mathcal{O}\).

We now aim to construct the Lie operad as an example of an operad following \cite[Ch. 4]{operads-and-props}. For constructing the Lie operad, let \(\mathcal{O}\) be an operad in the symmetric monoidal category \(\mathbf{Vect}\). By Definition \ref{def:operad-over-set}, \(\mathcal{O}\) is in particular a collection of vector spaces \(\mathcal{O}(n)\) for \(n \in \mathbb{N}\), with a \(S_{n}\) action on each \(\mathcal{O}(n)\). In fact, \(\mathcal{O}\) has a richer structure that this since Definition \ref{def:operad-over-set} also requires \(\mathcal{O}\) to come equipped with composition functions satisfying some axioms, but forgetting this for now, the structure thus described by itself is called an \(S\)-module. There is the category \(S\text{-}\mathbf{mod}\) of \(S\)-modules whose morphisms are \(S\)-module morphisms: morphisms that commute with the action of \(S_{n}\).

Similarly, we write the category \(\mathbf{Oper}(\mathbf{Vect})\) of operads in the symmetric monoidal category \(\mathbf{Vect}\). The morphisms from operads \(\mathcal{O}\) to \(\mathcal{P}\) are sequences of linear maps \(\mathcal{O}(n) \to \mathcal{P}(n)\), \(n \in \mathbb{N}\) that preserve the identity, preserve composition and commute with the action of \(S_{n}\).

As such, there is a natural forgetful functor \(\square: \mathbf{Oper}(\mathbf{Vect}) \to S\text{-}\mathbf{mod}\). It has a left-adjoint free operad functor \(\Lambda: S\text{-}\mathbf{mod} \to \mathbf{Oper}(\mathbf{Vect})\). For details of this construction we refer the reader to \cite{operads-and-props}. 

We can build an \(S\)-module from a single generating operation. For example, let 
\[
	\alpha
	=
	\def\svgscale{0.2}
...
By abuse of notation we denote by \(\alpha\) the \(S\)-module in which each \(\mathcal{O}(n)\) is the set of iterated compositions of \(\alpha\). For example
\[
	\def\svgscale{0.2}
...
With this in mind, we can describe the Lie operad in a simple graphical presentation in terms of generators and relations

\begin{example}
	\label{ex:lie-operad}
	The \textbf{Lie operad} over the symmetric monoidal category \(\mathbf{Vect}\), denoted \(\mathcal{O}_{\text{Lie}}\) is defined as
	\[
		\mathcal{O}_{\text{Lie}}
		\quad
		=
...
\begin{proposition}
	The algebras over the Lie operad over the symmetric monoidal category \(\mathbf{Vect}\), \(\mathcal{O}_{\text{Lie}}\) are exactly the Lie algebras.
\end{proposition}
\begin{proof}[sketch]
	Being objects of \(\mathbf{Vect}\), the algebras over Lie are vector spaces. The relations of example \ref{ex:lie-operad} provide the axioms of antisymmetry and the Jacobi identity. Since \(\mathcal{O}_{\text{Lie}}\) is constructed as a quotient of the free operad functor on a single generating binary operation, there are no additional relations than those that can be deduced from antisymmetry and the Jacobi identity.
\end{proof}

Recall that the goal of the Hinich--Vaintrob construction is to realise \(\mathcal{A}\) as the universal enveloping algebra (or an appropriate generalisation thereof) of a Lie algebra object in some symmetric monoidal category. Indeed, \(\mathcal{A}\) is a bialgebra, and Hinich--Vaintrob construct an object which is categorically a universal enveloping algebra, has a compatible coalgebra structure, and as a bialgebra is isomorphic to \(\mathcal{A}\).

The first step is to create a category whose morphisms encode the internal structure of \(\mathcal{A}\), i.e. the \ref{eq:AS} and \ref{eq:IHX} relations, but not yet with a choice of representation or the \ref{eq:STU} relations. An important feature of \(\mathcal{A}\) that enabled the construction of Lie algebra weight systems was that contraction of tensors corresponding to the Lie bracket or metric/casimir elements was akin to joining parts of diagrams in \(\mathcal{A}\). So a contraction-like operation would be a good start.

Indeed, composition in operads is reminiscent of contraction in a symmetric monoidal category, with the caveat that there are restrictions on what can be contracted: only outputs with inputs. Furthermore, in an operad, every operation has exactly one output, so taking the tensor product to produce an operaton with two outputs in not a natural operation.

A step towards describing the internal structure of \(\mathcal{A}\) without specifying a Lie algebra is to allow multiple outputs, thereby considering not just operations with a single output, but with multiple outputs. Operads are abstractions of the operations on some object (say, \(A\)), i.e. morphisms \(A^{\otimes n} \to A\). Slightly more generally, props are abstractions of all maps \(A^{\otimes n} \to A^{\otimes m}\).

\begin{definition}
	\label{def:prop}
	A \textbf{prop} is a symmetric monoidal category where every object is of the form \(\mathbf{1}^{\otimes n}\) for some object \(\mathbf{1}\). Equivalently, it's a symmetric monoidal category generated by a single object.
	Since props are categories, we use a boldface capital letter to denote them. We denote the monoidal unit by \(\mathbf{0}\), the generating object by \(\mathbf{1}\), and the object \(\mathbf{1}^{\otimes n}\) by \(\mathbf{n}\).
\end{definition}
...
\begin{remark}
	...
	\[
		...
		\end{tikzcd}
	\]
	Here \(\mathcal{F}\) is the left-adjoint and \(\mathbf{E}\) is the right-adjoint.

	The forgetful functor \(\mathcal{F}\) takes a prop \(\mathbf{P}\) to the operad whose operations are the \((\mathbf{m}, \mathbf{1})\)-ary elements of the prop (i.e. morphisms with \(m\) inputs and one output):
	...
	(here the outermost direct sum is over all functions \(f\) from \(\{1, \dots, m\} \to \{1, \dots, n\}\)).
	The prop \(\mathbf{E}(\mathcal{O})\) is also referred to as the \textbf{envelope of the operad \(\mathcal{O}\)}.
\end{remark}

\begin{proposition}
	The notions of algebra over a prop and algebra over an operad are equivalent in the sense that \(A \in \mathcal{C}\) is an algebra over the operad \(\mathcal{O}\) if and only if it is an algebra over the prop \(\mathbf{E}(\mathcal{O})\).
\end{proposition}

Now the main difference between contraction and partial composition in a prop is the marking of inputs and outputs. With metric Lie algebras, co- and contravariant indices could be contracted with the metric or casimir to invert their variance, but in a general prop there is no way to turn an input into an output.

Recall that the action of the symmetric groups \(S_{n}\) on an operad come from the symmetry of the symmetric monoidal category \(\mathcal{C}\). But this only permutes inputs amongst themselves, rather than permuting inputs with outputs. In a certain kind of operad, called a cyclic operad, permutations of inputs with outputs can be effectively achieved, and the distinction between inputs and outputs is lost. This is achieved, for example by the metric in a metric Lie algebra which allows it to be identified with its dual and therefore the distinction between co- and contra- variant elements of the tensor algebra is lost.

\begin{definition}
	\label{def:cyclic-operad}
	A \textbf{cyclic operad} in \(\mathcal{C}\) is an operad equipped with a cyclic structure. A \textbf{cyclic structure} is an action of each symmetric group \(S_{n + 1}\) on \(\mathcal{O}(n)\) which extends the \(S_{n}\) action on \(\mathcal{O}(n)\) and satisfies the cyclic cation condition below.

	First we define what we mean by an extension of the \(S_{n}\) action. Formally, let \(S_{n}\) be the group of permutations of the set \(\{1, \cdots, n\}\). The operad already has an action of \(S_{n}\) on each \(\mathcal{O}(n)\) which agrees with the action of the symmetry \(\tau\) of \(\mathcal{C}\).

	Let \(S_{n}^{+} \cong S_{n + 1}\) be the group of permutations of \(\{0, \cdots n\}\), and note that the subgroup of \(S_{n}^{+}\) that fixes \(0\) is isomorphic to \(S_{n}\). We say an action of \(S_{n}^{+}\) extends the action of \(S_{n}\) if the restriction of the action to \(S_{n} \subseteq S_{n}^{+}\) agrees with the action of \(S_{n}\) on \(\mathcal{O}(n)\).

	For each \(n \leq 1\), let \(\sigma_{c} \in S_{n + 1}\) denote the cyclic permutation sending \(0 \mapsto 1, 1 \mapsto 2, \cdots, n \mapsto 0\). We abuse notation by not making explicit in the name of \(\sigma_{c}\) the size \(n + 1\) of the symmetric group \(S_{n + 1}\) on which \(\sigma_{c}\) acts as this can be read from context. To be a cyclic operad, the action of \(S_{n + 1}\) must satisfy the \textbf{cyclic action condition}
...
If we allow the action of \(\sigma_{c}\) to be pictured as
\[
	\def\svgscale{0.3}
	\adjustbox{valign=c}{\input{graphics/cyclic_operad_action_definition_lhs.pdf_tex}}
	\quad
	=
	\quad
	\def\svgscale{0.3}
	\adjustbox{valign=c}{\input{graphics/cyclic_operad_action_definition_rhs.pdf_tex}}
\]
then the relation becomes
\[
	\def\svgscale{0.3}
	\adjustbox{valign=c}{\input{graphics/cyclic_operad_picture_lhs.pdf_tex}}
	\quad
	=
	\quad
	\def\svgscale{0.3}
	\adjustbox{valign=c}{\input{graphics/cyclic_operad_picture_rhs.pdf_tex}}
\]
so the cyclic action condition is natural to consider when trying to cyclically permute the inputs and outputs of the operations in an operad.

The following definition generalises the definition of a metric Lie algebra (which is an algebra over the cyclic operad object \(\mathcal{O}_{\text{Lie}}\)) to an arbitrary cyclic operad.

\begin{definition}
	\label{def:metric-algebra-over-cyclic-operad}
	For a cyclic operad \(\mathcal{O}^{\text{cyc}}\) in category \(\mathcal{C}\), a \textbf{metric algebra} over \(\mathcal{O}^{\text{cyc}}\) is a pair \((A, m)\) of a choice of object \(A \in \mathcal{C}\) which is an algebra over \(\mathcal{O}^{\text{cyc}}\) together with a symmetric morphism \(m: A \otimes A \to k\), such that (informally) using \(m\) to turn the output into an input and using its adjoint \(c\) to turn the last input into an output implements the action of the cyclic operad. Formally:
	\begin{itemize}
		\item there exists a morphism \(c: k \to A \otimes A\) such that the composition
			...
			is the identity (\(m\) has an \textbf{adjoint}).
		\item the composition
			\[\mathcal{O}(n) \otimes A^{\otimes n + 1} \longrightarrow A \otimes A \overset{m}{\longrightarrow} k\]
			is invariant under the cyclic subgroup \(C_{n + 1}\) of \(S_{n + 1}\).
	\end{itemize}
\end{definition}
...
\begin{definition}
	...
	\begin{itemize}
		\item The morphisms \(m\) and \(c\) are are symmetric and mutually adjoint (in the sense of Definition \ref{def:metric-algebra-over-cyclic-operad}).
		...
		\[\begin{tikzcd}
			...
			\\
			{\mathbf{n + 2}} && {\mathbf{3}}
			\arrow["{f\sigma_{c}}", from=1-1, to=1-3]
...
Using this prop, we can generalise the notion of a metric Lie algebra to an arbitrary symmetric monoidal category \(\mathcal{C}\).

\begin{definition}
	\label{def:metric-lie-algebra-object}
	Let \((\mathcal{C}, \tau)\) be a symmetric monoidal category and \(\mathcal{O}_{\text{Lie}}^{\text{cyc}}\) the Lie operad in \(\mathcal{C}\). Then the \textbf{metric Lie algebra objects in \(\mathcal{C}\)} are the algebras over the prop \(\mathbf{E}_{m}(\mathcal{O}_{\text{Lie}}^{\text{cyc}})\).
\end{definition}

Indeed a metric Lie algebra object in any symmetric monoidal category gives rise to a weight system. However, we can be slightly more general with the type of objects we permit, and still product systems. This is is necessary for the universal construction of Hinnich and Vaintrob, and will be nevessary for our generalisation thereof.

\begin{definition}
	\label{def:casimir-envelope-of-cyclic-operad}
	The \textbf{casimir envelope of the cyclic operad} \(\mathcal{O}^{\text{cyc}}\), denoted \(\mathbf{E}_{c}(\mathcal{O}^{\text{cyc}})\) is the prop generated by the prop \(\mathbf{E}(\mathcal{O}^{\text{cyc}})\) along with the element \(c \in \mathbf{E}_{c}(\mathcal{O}^{\text{cyc}})(\mathbf{0}, \mathbf{2})\) such that:
	\begin{itemize}
...
\begin{definition}
	\label{def:casimir-lie-algebra-object}
	Let \((\mathcal{C}, \tau)\) be a symmetric monoidal category and \(\mathcal{O}_{\text{Lie}}^{\text{cyc}}\) the Lie operad in \(\mathcal{C}\). Then the \textbf{casimir Lie algebra objects} are the algebras over the prop \(\mathbf{E}_{c}(\mathcal{O}_{\text{Lie}}^{\text{cyc}})\).
\end{definition}

This generalises the notion of a Lie algebra with a casimir element (but not necessarily with a metric). We call such an object a casimir Lie algebra object.

The reason we did not see an analagous definition earlier in this chapter is the following. In the case of finite-dimensional Lie algebras, every casimir Lie algebra is a metric Lie algebra. This is because in finite-dimensional vector spaces, a casimir, being an invariant symmetric map \(c: k \to A \otimes A\) is always has an adjoint, and its adjoint is a metric. So, any rigid casimir Lie algebra object is a metric one. For example, finite-dimensional casimir Lie algebras, superalgebras, etc.
...
The reason to consider casimir Lie algebra objects as opposed to just metric Lie algebra objects to make the universal construction is the following. Consider specifying a metric Lie algebra object \(\mathfrak{g}\) in some symmetric monoidal category \(\mathcal{C}\). Then the weight system \(\mathcal{W}_{\mathfrak{g}}\) has a relation of the form
	\[
		...
		\mu
	\]
for some scalar \(\mu\). Here the first diagram is formed by pairing the casimir \(c\) with the metric \(m\). This was discussed in Remark \ref{rem:metric-casimir-bubble-is-dimension}. For example if \(\mathcal{C} = \mathbf{Vect}\), this is the dimension of the Lie algebra, or if \(\mathcal{C} = \mathbf{sVect}\) it is the superdimension. This relation is just one example of a relation between diagrams with no univalent vertices to scalars.

In \(\mathcal{A}\) there are no such relations. In fact, there are no chord diagrams that have only trivalent vertices and no univalent vertices. So, metric Lie algebras are the wrong objects to consider if we want to find a bialgebra isomorphic to \(\mathcal{A}\). Instead, algebras having only casimirs should be considered.

\begin{definition}
	The \textbf{universal Casimir Lie algebra object} is the generating object \(\mathbf{1}\) in the symmetric monoidal category \(\mathbf{E}_{c}(\mathcal{O}^{\text{cyc}}_{\text{Lie}})\). We denote this as \(\mathbf{1}_{c} \in \mathbf{E}_{c}(\mathcal{O}^{\text{cyc}}_{\text{Lie}})\).
\end{definition}

This object \(\mathbf{1}_{c} \in \mathbf{E}_{c}(\mathcal{O}_{\text{Lie}}^{\text{cyc}})\) is the object to which Hinich--Vaintrob apply the following construction which generalises the notion of a universal enveloping algebra. As in the construction of the classical universal enveloping algebra, this proceeds via constructing a generalisation of the tensor algebra.
...
\begin{definitions}
		...
		\[
			...
			\end{array}
		\]
		where \(k\) is the tensor unit in \(\mathcal{C}\).
\end{definitions}
\begin{definitions}
	...
	Let \(o_{a} \otimes_{S_{n}} \alpha \in \mathcal{O}(n + 1) \otimes_{S_{n}} \Gamma(A^{\otimes n})\), and \(o_{b} \otimes_{S_{n}} \beta \in \mathcal{O}(m + 1) \otimes_{S_{n}} \Gamma(A^{\otimes m})\). Their product is defined by the following operad composition
	\[
		(o_{a} \otimes_{S_{n}} \alpha) \cdot (o_{b} \otimes_{S_{n}} \beta) = (o_{a} \circ_{1} o_{b}) \otimes_{S_{n}} (\alpha \otimes \beta).
	\]
	Diagramatically this composition looks like
	\[
		\def\svgscale{0.35}
...
and applying the global section functor, we have maps
...
\begin{example}
	...
	\[
		...
		\adjustbox{valign=c}{\input{graphics/external_enveloping_algebra_lie_example_factor_1.pdf_tex}}
	\]
	in \(U(\mathcal{O}_{\text{Lie}}, \mathfrak{g})\). Alternatively, written as a formula, we have
	\[[\xi_{1}, \xi_{2}] = \xi_{1} \cdot \xi_{2} - \xi_{2} - \xi_{1}\]
	which is exactly the defining relation of a classical universal enveloping algebra.
\end{example}

Let \(\mathcal{O}_{\text{Lie}}\) denote the Lie algebra operad over the prop (so therefore symmetric monoidal category) \(\mathbf{E}_{c}(\mathcal{O}^{\text{cyc}}_{\text{Lie}})\). Recall that \(\mathbf{E}_{c}(\mathcal{O}^{cyc}_{Lie}\) is formed from adding the morphism \(c\) to \(\mathbf{E}(\mathcal{O})\), and the only additional relations imposed involve \(c\), therefore by Definition \ref{def:algebra-over-prop}, \(\mathbf{1}_{c}\) is an algebra over the operad \(\mathcal{O}_{\text{Lie}}\) over the symmetric monoidal category \(\mathbf{E}_{c}(\mathcal{O}^{\text{cyc}}_{\text{Lie}})\)

\begin{theorem}[\cite{cyclic-operads-and-the-algebra-of-chord-diagrams}]
	\label{thm:universal-casimir-lie-envelope-chord-diagrams-algebra-isomorphism}
	Let \(U(\mathcal{O}_{\text{Lie}})\) denote the Lie operad over the symmetric monoidal category \(\mathbf{E}_{c}(\mathcal{O}^{\text{cyc}}_{\text{Lie}})\). The algebra \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c})\) is isomorphic as an algebra to \(\mathcal{A}\).
\end{theorem}
...
\begin{lemma}
	...
	and \(\pi \in kS_{n}\), the group algebra of \(S_{n}\) with coefficients in the monoidal unit of \(\mathcal{C}\).
\end{lemma}

\begin{proof}
	Recall from the definition of an operad, there is a \(S_{n}\)-action on \(\mathcal{O}_{\text{Lie}}(n)\). We proceed by induction.
...
\begin{proofof}{Theorem \ref{thm:universal-casimir-lie-envelope-chord-diagrams-algebra-isomorphism}}
	...
	\]
	A morphism \(E \in \mathbf{E}_{c}(\mathbf{0}, \mathbf{n})\) is associated to a diagram in \(\mathcal{A}\); choose a presentation for \(E\) in the symmetric monoidal category \(\mathbf{E}_{c}(\mathbf{0}, \mathbf{n})\), this determines a unitrivalent graph with a cyclic order on the internal vertices, and a linear order on the univalent vertices induced by the order \(\mathbf{1} < \mathbf{2} < \cdots < \mathbf{n}\). By Proposition \ref{prop:linear-jacobi-diagrams-cyclically-invariant}, this map factors through the natural projection from linear orders on univalent vertices to cyclic orders. This therefore defines a map \(f: T(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c}) \to \mathcal{A}\).
	...
	The map \(f\) is surjective: we claim every Jacobi diagram can be presented by a graph whose edges do not have local minima. If the Jacobi diagram is genus zero, so a tree, then this is obvious. Suppose the Jacobi diagram as genus \(m\). Then by removing \(m\) short arcs from \(m\) of its edges, there is a way to obtain a modified diagram which is again a tree. This can be presented without local minima, and can be restored to a diagram equivalent to the original diagram by attaching casimirs to connect those edges that were formed by the removal of short arcs, a process which doesn't ontroduce local minima, proving the claim.

	The map \(f\) is injective because \(\ref{eq:STU}\) is a generating relation for \(\mathcal{A}\) (Proposition \ref{prop:stu-implies-as-ihx}) and its preimages in \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c})\) are related by the relation in Example \ref{ex:stu-in-external-enveloping-algebra}. This proves the algebra isomorphism.
\end{proofof}

We turn to the coalgebra structure of \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c})\) and the bialgebra isomorphism to \(\mathcal{A}\). Recall from Proposition \ref{prop:jacobi-diagram-coproduct}
% and Corollary \ref{cor:primitives-equals-connected-jacobi-diagrams} 
that the coproduct structure on \(\mathcal{A}\) is given by the formula
\[\Delta(A) = \sum_{C \subset S} A_{C} \otimes A_{\overline{C}}\]
where \(S\) is the set of connected components of \(A\) (not considering the skeleton), \(\overline{C} = S \smallsetminus C\), and \(A_{C}\) is the Jacobi diagram consisting of only the subset \(C\) of connected components.

Since in the map \(f: U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c}) \to \mathcal{A}\), the path from the leftmost leaf to the root becomes the skeleton, the correct notion of primitive in \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c})\) is connected components, excluding the path from the leftmost leaf to the root.

\begin{definition}
	\label{def:coproduct-on-generalised-enveloping-algebra}
	The coproduct \(\Delta\) on \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c})\) is defined by pulling the coproduct on \(\mathcal{A}\) back through the algebra isomorphism of Theorem \ref{thm:universal-casimir-lie-envelope-chord-diagrams-algebra-isomorphism}.
\end{definition}

\begin{remark}
	In \cite{cyclic-operads-and-the-algebra-of-chord-diagrams} a operad-theoretic characterisation of the primitive elements is given in terms of modular operads. Recalling from Corollary \ref{cor:primitives-equals-connected-jacobi-diagrams} that by the formula for the coproduct, the primitive elements are exactly the connected components, this also gives a definition of the coproduct without reference to connected components.
\end{remark}

With this definition, we obtain as a corollary an upgrade of the isomorphism of Theorem \ref{thm:universal-casimir-lie-envelope-chord-diagrams-algebra-isomorphism} to the level of bialgebras.

\begin{theorem}[\cite{cyclic-operads-and-the-algebra-of-chord-diagrams}]
	\label{thm:universal-casimir-lie-envelope-chord-diagrams-bialgebra-isomorphism}
	The bialgebra \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c})\) is isomorphic as a bialgebra to \(\mathcal{A}\).
\end{theorem}

\begin{proof}
	Recall that in the map \(f: U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c}) \to \mathcal{A}\) of Theorem \ref{thm:universal-casimir-lie-envelope-chord-diagrams-algebra-isomorphism}, the path from the leftmost leaf to the root of an element of \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c})\) becomes the skeleton of a chord diagram in \(\mathcal{A}\). Therefore connected components in \(\mathcal{A}\) not considering the skeleton map to connected components in \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c})\) not considering the path from the leftmost leaf to the root. As such, the formula for the coproduct in both spaces is the same.
\end{proof}

Theorem \ref{thm:universal-casimir-lie-envelope-chord-diagrams-bialgebra-isomorphism} is a formalisation of the statement that ``\(\mathcal{A}\) behaves like the universal enveloping algebra of a Lie algebra''. In the next chapter we look at a generalisation of this statement in the context of welded knots.
