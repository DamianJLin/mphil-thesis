...
\begin{definitions}
	\begin{enumerate}
		...
		\item A monoidal category \(\mathcal{C}\) is \textbf{rigid} if for every object \(A\) there exists an object \(A^{\vee}\) and a pair of morphisms \(\iota: A^{\vee} \otimes A \to k\) and \(\epsilon: k \to A^{\vee} \otimes A\) --- respectively called the \textbf{evaluation} and \textbf{coevaluation} --- such that
			\[(\id_{A} \circ \epsilon) \circ (\iota_{A} \circ \id_{A}) = \id_{A}\]
			and
			\[(\epsilon \circ \id_{A}) \circ (\id_{A} \circ \iota_{A}) = \id_{A^{\vee}}\]
			Diagramatically, these are
			\[
