...
\begin{proof}
	...
	We can split \(\Theta\) up based on which connected component the marked vertex now connects to. For connected components other than the connected component of the marked vertex, apply a vertical version of the generalised \ref{eq:IHX} relation, 
    %TODO (Zsuzsi) Reference Proposition 2.1.6 rather than (or in addition to) the original IHX
    where the the whole connected component is inside the grey box
    %TODO (Zsuzsi) I was looking for a grey box and found this very confusing because there was no picture corresponding to this paragraph. Then I realised you're referring to the grey box in Prop 2.1.6. A picture would help.
    except for where the univalent vertices connect to the bottom line. This is the case of the generalised \ref{eq:IHX} where \(n = 0\) and no vertices leave. Hence, the sum vanishes for that connected component.
    %TODO (Zsuzsi) I can sort of figure out what this means because I use the Generalised IHX daily. But it's not super clear.
	...
	\]
    %TODO (Zsuzsi) The last sentence is not a sentence. Also, where does the dotted end go? What are you summing over? Why is it equal to the other one? 
    ...
    %TODO (Zsuzsi) You haven't defined balloon but I get it. It would be more precise to show the "balloon = 0" relation here.
...
Conjecture \ref{conj:lie-algebra-weight-systems-span} being false means that the \ref{eq:STU} relation describes more than the relation \(\rho([x, y]) = \rho(x)\rho(y) - \rho(y)\rho(x)\) for a representation \(\rho\) of a metric Lie algebra. In fact, Construction \ref{cons:lie-algebra-weight-system} 
%TODO (Zsuzsi) "of Lie algebra weight systems"
is just one example of a more general construction introduced by Vogel and Vaintrob with the purpose of constructing weight systems coming from metric Lie super-algebras, and further generalisations of Lie algebras.
...
\begin{definition}
	...
	\]
    %TODO (Zsuzsi) The second of these can't be right
...
\begin{definitions}
	...
	\end{enumerate}
%TODO (Zsuzsi) The last sentence (with several formulas embedded in it) is not a grammatical sentence 
\end{definitions}
...
Furthermore, if the monoidal category \(\mathcal{C}\) is additive and the functor \(\otimes: \mathcal{C} \times \mathcal{C} \to \mathcal{C}\) is bilinear, then Lie algebra objects internal to \(\mathcal{C}\) can be defined.

\begin{definitions}
	\label{def:internal-to-additive-monoidal-category}
	\begin{enumerate}
		\item A \textbf{Lie algebra object} in \(\mathcal{C}\) is an object \(L\) equipped with a bracket morphism \(\beta : \mathcal{C} \otimes \mathcal{C} \to \mathcal{C}\) such that
			\[\left( \beta \circ (\beta \otimes \operatorname{id}) \right) \circ (1 + \tau_{123} + (\tau_{123})^{2}) = 0 \qquad \text{and} \qquad \beta + \beta \circ \tau = 0.\]
			%TODO (Zsuzsi) You could call these the Jacobi and Antisymmetry relations (help the reader out a little)
			...
			\[
				...
				0.
			\]
			Here the trivalent vertex corresponds to the generalised bracket morphism \(\beta\).
            %TODO (Zsuzsi) Why generalised? (above you called it just bracket)
		\item A (right) \textbf{module \(\rho\) over a Lie algebra object \(L\) in \(\mathcal{C}\)} is an object \(M\) with a morphism \(\rho: M \otimes L \to M\), 
        %TODO (Zsuzsi) Call $\rho$ the action morphism
        diagramatically denoted
			\[
			...
			\[
				...
				.
			\]
            %TODO (Zsuzsi) One of these pictures whould have a cross (swap of L and M) on top: in order for two pictures to be equal, the endpoints must be in the same positions. This should also help sort out your later question about the minus sign.
        ...
        %TODO (Zsuzsi) somewhere before this you should say that Lie algebra objects in Vect are the actual Lie algebras
        by satisfying
			...
			\[
				...
				\includegraphics[width=0.06\textwidth, valign=c]{graphics/string_diagram_casimir_no_dot.pdf}\ .
			\]
			(like before these morphisms allow us to drop arrows on the thin lines) 
            %TODO I don't know, there haven't been any duals here and the arrows have been always going top to bottom... so, was there a need for arrows in the first place?
            satisfying
			\[
...
\[
	...
	.
\]
%TODO (Zsuzsi) Why did these get flipped upside down compared to the definition of a right module? In this picture, I don't understand the action: what acts on what?
\begin{shaded}
...
\end{shaded}
%TODO (Zsuzsi) Yes, and I suspect it will be easier once you clarify whether you are acting on the object or its dual, and what does it mean to act on the dual. Are you turning a right action on $M$ into a left action on $M^\vee%? Or on $M$? Morphisms should always go top to bottom, and here that doesn't seem to be the case. (Or always bottom to top, but there should be one convention.)
...
%TODO (Zsuzsi) I don't want to give you too much editing work, but the sequence of definitions above would really benefit from some grounding examples. Maybe you could do some minimal rearranging and introduce the examples of even just Vect and SVect sooner, sort of as a running example that could illustrate everything.
...
\begin{theorem}[\cite{vassiliev-knot-invariants-and-lie-s-algebras}]
    ...
    %TODO (Zsuzsi) You haven't said that modules over $L$ form a monoidal category, and how.
...
\begin{example}
	...
	\[[x, y] = - (-1)^{ij}[y, x]\]
	%TODO (Zsuzsi) Start a new sentence
    ...
    %TODO (Zsuzsi) Explain why these are the Lie algebra objects in SVect.
\end{example}
...
We return to the universal Lie algebra of Vaintrob which was discussed in the previous section. 
%TODO (Zsuzsi) Did you discuss this in the previous section? I recommend inserting a specific reference, like "Remark X.Y", or a reminder of what the goal is.
It is yet uncertain 
%TODO (Zsuzsi) "Yet uncertain" = "at this point in your thesis", or "at this point ion the history of mathematics"
to what degree such an algebra exists. The obstruction are the zero divisors in Vogel's algebra \(\Lambda\), \cite{can-yang-baxter-imply-lie-algebra, on-refined-vogels-universality, construction-of-lie-algebra-weight-system-kernel-via-vogel-algebra} give more details.
%TODO (Zsuzsi) As far as I can tell, you never mentioned $\Lambda$ before, so we don't know what it is or what it does. I think this sentence gives an unhelpful amount of detail: the reader thinks they're missing something, and starts flipping back to see where they missed it.

There is, however, a theoretical construction of 
Hinnich-Vaintrob \cite{cyclic-operads-and-the-algebra-of-chord-diagrams}: they construct a Lie algebra object in a symmetric monoidal category with a weight system which is isomorphic to \(\mathcal{A}\). This guarantees that every Vassiliev invariant can be constructed from applying Construction \ref{cons:lie-algebra-weight-system} to some Lie algebra object in some symmetric monoidal category. However, it doesn't give any hints as to which symmetric monoidal category one needs to work in to construct a particular Vassiliev invariant. Thus, it is useful theoretically, but not computationally. In this section we review the Hinic--Vaintrob construction, and in Chapter \ref{ch:welded-knots-isometry-lie-algebras-and-arrow-diagrams}, we will generalise it from Vassiliev invariants of classical knots to those of welded long knots.

%TODO (Zsuzsi) This paragraph needs a stransition sentence at the beginning, something like: "An essetntial concept to the HV construction is that of an operad, which formally captures the general behaviour of associative operations in a range of abstract algebraic contexts."
The operations on a set can always be composed by taking as input the output of another operation. These compositions always obey certain rules. If the set is a vector space, the operations too form a vector space. 
%TODO (Zsuzsi) Any operations? I suggest: "For example, if the set is a vector space $V$, then linear maps on $V$ also form a vector space."
An operad captures this concept of operations on some object of a category abstractly. 
%TODO (Zsuzsi) Associative only. I would also avoid saying "on some object of a category" to avoid confusion, because below the objects in your category are sets of operations. You could instead say on an "algebraic stucture" or just "structure".
...
\begin{definition}
	\label{def:operad}
	An \textbf{operad} \(\mathcal{O}\) in a monoidal category \(\mathcal{C}\) consists of:
	...
	\begin{itemize}
		...
		\item a series of composition morphisms:
        %TODO (Zsuzsi) bold "composition morphims"?
	...
	satisfying the following axioms. (Note: the axioms are listed for completeness, but won't be used heavily below.)
		\begin{enumerate}
			...
			\item \textbf{Associativity of composition}, that for \(\theta \in \mathcal{O}(n)\), and \(\theta_{1}\in \mathcal{O}(k_{1})\), \(\theta_{2} \in \mathcal{O}(k_{2})\), up to \(\theta_{n} \in \mathcal{O}(k_{n})\), the two ways of forming the composition of the \(\theta_{i, j}\) into \(\theta_{i}\) into \(\theta\) 
            %TODO (Zsuzsi) Confusing: you didn't introduce double indices above, but tehre are double indices here and in the formulas below. Switched from tensor to commas: tensor product or direct sum? If it's a tensor product, write $\otimes$ rather than $(,,,)$. (Or maybe I'd write it as a product if it must be shortened.)
            are the same:
            ...
            %TODO (Zsuzsi) you didn't say above that $\mathcal{C}$ was symmetric
		\end{enumerate}
\end{definition}
...
\begin{definition}
    ...
    %TODO (Zsuzsi) This is not a definition: if it's in a definition environment, it should be defined. Otherwise, it should be just exposition (not in an environment). On the math: it's not $A$ that satisfies compatibility conditions, but the operations on $A$. At the very least one should write down these operations on $A$ (as maps $A\otimes A \otimes ...\otimes A \to A$), and then if you want to be informal, say that they behave as the "operations" in $\mathcal{O}$.
\end{definition}

In essence, an algebra over an operad is an object in \(\mathcal{C}\) whose operations obey the rules defined by \(\mathcal{O}\). 
%TODO (Zsuzsi) This sentence could come before the formal definition.

\begin{example}
	\label{ex:lie-operad}
	Let \(\Lambda\) denote the free operad functor (for details we refer the reader to \cite{operads-and-props}). 
    %TODO (Zsuzsi) You can refer for details, but you should say something about what this is. From what category to what category? Intuitively, what is it?
    %TODO (Zsuzsi) On further thought: Basically you're using generators and relations, which, if you want to be precise and technical, depends on three things: the Free functor, generation within an operad, and taking a quotient. You don't explain any of the three, and I agree that someone who knows some algebra can kindof imagine. I think it would be more reader friendly to own this and say that one can use generators and relations for operads, which work pretty much how one would expect, and then write it as generators and relations, like $\langle gens | rels\rangle$. Or, one can be precise, but then one should define the things.
    Let
	\[
		\alpha
		=
...
Recall that the goal is an algebra isomorphic to \(\mathcal{A}\). 
%TODO (Zsuzsi) The goal is more than that, like, an easier algebra isomorphic to $\mathcal{A}$ is $\mathcal{A}$... Really, the goal is a Lie algebra object in a monoidal category, whose universal enveloping algebra (in the appropriate sense) is isomorphic to $\mathcal A$.
The first step is to create an object that encodes the internal structure of \(\mathcal{A}\), i.e. the \ref{eq:AS} and \ref{eq:IHX} relations, but not yet with a choice of representation or the \ref{eq:STU} relations. An important feature of \(\mathcal{A}\) that enabled the construction of Lie algebra weight systems was that contraction of tensors corresponding to the Lie bracket or metric/casimir elements was akin to joining parts of diagrams in \(\mathcal{A}\). So a contraction-like operation would be a good start.

Composition in operads is something like contraction in a symmetric monoidal category, but with restictions on what can be contracted: only outputs with inputs. Furthermore, in an operad, every operation has exactly one output, and as a result there is no natural notion of tensor product in an operad.

A step towards describing the internal structure of \(\mathcal{A}\) without specifying a Lie algebra is to allow multiple outputs, thereby considering not just operations but, for example in \(\mathcal{C} = \mathbf{Vect}\), general multilinear functions.
%TODO (Zsuzsi) Multilinear functions have only one output?
Operads were abstractions of the operations on some object (say, \(A\)), i.e. morphisms \(A^{\otimes n} \to A\). Slightly more generally, props are abstractions of all maps \(A^{\otimes n} \to A^{\otimes m}\).

\begin{definition}
	\label{def:prop}
<<<<<<< Updated upstream
	A \textbf{prop} is a symmetric monoidal category where every object is of the form \(A^{\otimes n}\) for some object \(A\). 
    %TODO (Zsuzsi) could you not denote this by $A$, but use boldface 1 right away? In the next definition you denote by $A$ an algebra over a prop, and you send $!$ to $A$, which makes $A$ quite loaded.
    Equivalently, it's a symmetric monoidal category generated by a single object.
    Since props are categories, we use a boldface capital letter to denote them. In a prop, we denote the field \(k\) by \(\mathbf{0}\), 
    %TODO (Zsuzsi) there is no field in a general PROP, it's just the tensor unit
    the generating object \(A\) by \(\mathbf{1}\), and the object \(A^{\otimes n}\) by \(\mathbf{n}\).
=======
	...
	Since props are categories, we will use a boldface capital letter to denote them. In a prop, we denote the monoidal unit \(k\) by \(\mathbf{0}\), the generating object \(A\) by \(\mathbf{1}\), and the object \(A^{\otimes n}\) by \(\mathbf{n}\).
>>>>>>> Stashed changes
\end{definition}
...
Recall that the action of the symmetric  
group 
%TODO (Zsuzsi): In an operad?
comes from the symmetry of the symmetric monoidal category \(\mathcal{C}\). But this only permutes inputs amongst themselves, rather than permuting inputs with outputs. In a certain kind of operad, called a cyclic operad, permutations of inputs with outputs can be achieved.
%TODO (Zsuzsi): I would add something like: "effectively identifying inputs and outputs", or "erasing the differemce between inputs and outputs". Then add some motivation: "This is achieved, for example, by the metric in a Lie algebra, which allows it to be identified with its dual, or, equivalently, allows it to be contracted with itself."
...
\begin{definition}
	...
	Firs we define what we mean by an extension of the \(S_{n}\) action. Formally, let \(S_{n}\) be the group of permutations of the set \(\{1, \cdots, n\}\). The operad already has an action of \(S_{n}\) on each \(\mathcal{O}(n)\) which agrees with the action of the symmetry \(\tau\). 
    %TODO (Zsuzsi) What is $\tau$?
...
%TODO (Zsuzsi) Insert motivating sentence here, reiterating the inspiration from metric Lie algebras.
