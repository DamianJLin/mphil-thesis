...
\section{Jacobi diagrams, AS, STU and IHX} %TODO (Zsuzsi) This is a section title in the Dror tradition -- I'd consider just "Jacobi diagrams"
...
As usual, this is not an individual relation but a class of relations, true in any diagrams that are identical except for the subdiagrams being as shown.
...
\begin{proof}
	...
	\begin{enumerate}
		\item
			...
			If the vertex is not immediately adjacent to a univalent vertex, then it is some \(d\) vertices `in the way'.  %Todo (Zsuzsi) grammar
            By applying \ref{eq:STU} to those vertices yields a sum of \(2^{d}\) diagrams, all identical except for differing by \ref{eq:AS}, now on a vertex adjacent to a univalent vertex.
        ...
        %TODO (Zsuzsi) This proof is essentially a verbal description of pictures. It would be nicer to include and refer to the picture.
\end{enumerate}
\end{proof}
...
\begin{corollary}
	The bialgebras \(\mathcal{A}\) and \(\mathcal{J}\) are isomorphic. %TODO (Zsuzsi) This might just be me but I wouldn't write $J=A$, because the equal sign is loaded... I'd just say from here on we denote by A both chord diagrams and Jacobi diagrams.
\end{corollary}
...
Similar diagrammatic relations to \ref{eq:STU}, \ref{eq:AS} and \ref{eq:IHX} satisfied in \(\mathcal{A}\) are appear also in the context of a graphical notation for multilinear maps, a fact which we may exploit to probe \(\mathcal{A}\). Before seeing how, let us review this graphical notation following \cite{wheeling-a-diagrammatic-analogue-of-the-duflo-isomorphism, on-the-rozansky-witten-weight-systems}.

\begin{remark}
	This diagrammatic calculus is well-known but it goes by many names: string diagram calculus, Penrose calculus, tensor calculus, diagrammatic calculus for tensors, etc. We call them string diagrams. %TODO (Zsuzsi) I wouldn't necessarily put this in a remark environment, it could be part of your intro paragraph
\end{remark}

A tensor is a multilinear map \(X_{1} \otimes X_{2} \otimes \cdots \otimes X_{n} \to Y_{1} \otimes Y_{2} \otimes \cdots \otimes Y_{m}\), or equivalently (via the canonical isomorphism) an element of the vector space \(X_{1}^{\ast} \otimes X_{2}^{\ast} \otimes \cdots X_{n}^{\ast} \otimes Y_{1} \otimes Y_{2} \otimes \cdots \otimes Y_{m}\).
...
In fact, since the relation if true for any elements \(x, y \in \mathfrak{g}\), the labels on the edges can be dropped. %TODO (Zsuzsi) Clarify this sentence
...
Furthermore, suppose \(\mathfrak{g}\) is a metric Lie algebra. Then it has an invariant, nondegenerate, bilinear form \(\langle \cdot, \cdot \rangle \in \mathfrak{g}^{\ast} \otimes \mathfrak{g}^{\ast}\). Being nondegenerate, it can be inverted to an element \(c \in \mathfrak{g} \otimes \mathfrak{g}\). We have additional bivalent vertices %TODO (Zsuzsi) "These elements can be diagrammatically represented as additional bivalent vertices"
\[
...
\]
These can be used to change the arrow direction on any edge, allowing us to drop the edge arrows from the notation. %TODO (Zsuzsi) I would add that one way to view this is that the bilinear form induces an isomorphism of g-modules between g and g^*, hence you no longer need to keep track of which is which, hence no need for arrows
...
\]
A similar argument works for the casimir. %TODO (Zsuzsi) I think this is the first time you use the word casimir, perhaps introduce it earlier where you first talk about it

A representation of \(\mathfrak{g}\) into %TODO I think a representation is "on" not "into" V
a finite-dimensional vector space \(V\) can be written as a tensor \(\rho \in \mathfrak{g}^{\ast} \otimes V^{\ast} \otimes V\). This takes a new kind of input an output, namely a \(v \in V\) which we denote by a thick line at a shallow angle
\[
	\includegraphics[width=0.11\textwidth, valign=c]{graphics/representation_vertex.pdf}.
\]
The identity the action is a Lie action, 
%TODO (Zsuzsi) grammar
\[\rho([x, y]) = \rho(x)\rho(y) - \rho(y)\rho(x)\] is graphically
\[
...
\end{theorem}
%TODO (Zsuzsi) I would have an informal intro paragraph describing what the construction will do, and state the theorem after, when we can understand what it states. (My principle, which is mainstream but not universal, is that Theorem, Lemma and Definition environments should always include precise statements that the reader has enough information to understand.)

\begin{construction}
	\label{cons:lie-algebra-weight-system}
    %TODO Zsuzsi: Start with the input: a lie algebra and a chord diagram with this many trivalent vertices, this many univalent vertices, etc. Set the notation at the beginning. Say what output you're aiming for: an element of U(g)
	Given a Lie algebra \(\mathfrak{g}\), the vertex \(m\)
    %TODO (Zsuzsi) Are we supposed to know what $m$ is? I think above you called in $[,]$.
    with signature \(\mathfrak{g} \otimes \mathfrak{g} \otimes \mathfrak{g}\) is associated to a genuine 
    %TODO (Zsuzsi) what does genuine mean here?
    element of the tensor algebra \(T_{\mathfrak{g}}(m) \in \mathfrak{g}^{\otimes 3}\). For each trivalent vertex of the Jacobi diagram \(J \in \mathcal{A}\), take \(T_{\mathfrak{g}}(m)\), and take all of their tensor products, in arbitrary order. 
    %TODO (ZSuzsi) Streamline the sentence above
    There being \(v\) trivalent vertices, this lies in \(\mathfrak{g}^{\otimes 3v}\).
	...
	The number of univalent vertices matches the number of remaining uncontracted trivalent vertices, \(u = 3v - 2e\). In-particular the edges between univalent and trivalent vertices, along with the cyclic order on univalent vertices determines a cyclic order on the remaining trivalent vertices.
    %TODO (Zsuzsi) This is a bit unclear. The tensor factors were associated to the "ends" of the edges coming out of a trivalent vertex, not to the vertices. So, what do you mean by "remaining" or "uncontracted" trivalent vertices? In my opinion this is not needed: the remaining tensor factors correspond to the univalent vertices, period. And these are cyclically ordered. 
    So, a cyclic order on the factors of \(\mathfrak{g}^{\otimes 3v - 2e} = \mathfrak{g}^{\otimes u}\). Permute the factors into an order which respects the induced cyclic order (there are many --- choose one).
    %TODO (Zsuzsi) There are exactly $u$ choices?
    Projecting from the tensor algebra of \(\mathfrak{g}\) into the universal enveloping algebra \(\mathcal{U}(\mathfrak{g})\), we obtain an element \(W_{\mathfrak{g}}(J) \in \mathcal{U}(\mathfrak{g})\).

	A representation \(\rho: \mathfrak{g} \to \operatorname{Hom}(V)\) of \(\mathfrak{g}\) extends uniquely to a representation \(\rho: \mathcal{U}(\mathfrak{g}) \to \operatorname{Hom}(V)\) of \(\mathcal{U}(\mathfrak{g})\). Use this representation and take the trace of \(W_{\mathfrak{g}}(J)\). The result is an element of the ground field: \(W_{\mathfrak{g}, \rho}(J) = \operatorname{tr}(\rho(T_{\mathfrak{g}}(J))) \in \mathbb{C}\).
\end{construction}
...
\begin{warning}
    ...
    %TODO (Zsuzsi) This warning should be stated within the construction, rather than as a separate environment?
\end{warning}
...
%TODO (Zsuzsi) Proof of what?
	The contruction creates a string diagram which is well-defined modulo certain diagrammatic relations as seen at the start of this section such as the cyclic invariance of the contraction of the metric and bracket. 
    %TODO (Zsuzsi) Instead of "relations seen at the beginning of the sections such as ..." have the relations as numbered equations, and reference them specifically.
    By \cite[Proposition 3.1.1]{vassiliev-knot-invariants-and-lie-s-algebras}, string diagrams modulo these relations are exactly encoded by unitrivalent graphs with a cyclic order on trivalent vertices, and a linear order on univalent vertices.
	...
	Thus, to prove that the map \(W_{\mathfrak{g}}: \mathcal{A} \to \mathcal{U}(\mathfrak{g})\) is well-defined it suffices to show that the string diagrams obey the \(\ref{eq:STU}\) relations when interpreted as elements of \(\mathcal{U}(\mathfrak{g})\). 
    %TODO (Zsuzsi) And the cyclic symmetry?
	...
	Finally, a Jacobi diagram's univalent vertices only have a cyclic order. But in the construction a true order respecting that cyclic order was chosen. We need to show that any order respecting the cyclic order leads to the same result. It suffices to show that the tensor is cyclically symmetric. Indeed, a stronger statement is true: anything satisfying the \ref{eq:STU} relation is cyclically symmetric. 
    %TODO (Zsuzsi) I find this phrasing confusing because the tensor lives in the codomain, while "anything satisfying STU" lives in the domain. Maybe you just want to say that the required property follows from the fact that chord diagrams on a line are cyclically symmetric.
    Since it is stronger, let us prove it instead for \(\mathcal{A}\).
    ...
    %TODO (Zsuzsi) I think that's a good idea, so here in the proof you can just say this is a direct consequence of Lemma XX.

	It is equivalent to prove that any Jacobi diagram on a long line is cyclically symmetric. Examine the operation of taking the univalent vertex (which we will here call legs) 
    %TODO (Zsuzsi) Singular vs plural
    in the first position and moving it to the last position, reducing by one the positions of all other legs. 
    %TODO (Zsuzsi) I think saying you're reducing the positions of the legs is unnecessary (it's about as confusing as it is helpful)
    This generates the cyclic group on the number of univalent vertices, 
    %TODO (Zsuzsi) Maybe better: "This operation generates all cyclic permutations of the legs"?
    so it is enough to show this operation preserves the Jacobi diagram. The \ref{eq:STU} relation gives the cost
    %TODO (Zsuzsi) Cost is a word you and I and Dror use in conversation, but it isn't mathematically precise, nor universally understood
    of commuting the first leg past the \(i\)th leg: the same diagram with the first leg attached to the \(i\)th leg. Thus cost commuting the first leg to the very last place is the sum over the other legs of attaching it to those legs. 
    %TODO (Zsuzsi) Grammar
    We will show that this cost is zero.

	We split this sum by the connected components
    %TODO (Zsuzsi) Component? (singular)
    of the \(i\)th leg. On components that don't contain the first leg, this is the sum, over legs, of the same diagram, with the first leg attached to the \(i\)th leg. This vanishes by the generalised \ref{eq:IHX}. On the component that contains the first leg, generalised \ref{eq:IHX} produces a diagram with a loop at the top, which is killed by \ref{eq:AS}.
    %TODO (Zsuzsi) This is near impossible to decode without a picture
...
\begin{example}[Weight system for \({\mathfrak{sl}_{2}}\)]
	...
	(those pairings not displayed are zero). Here we have chosen the normalisation of the metric that agrees with the trace in the adjoint representation.
...
In \cite{remarks-on-the-vassiliev-knot-invariants-coming-from-sl2}, the following skein relation is derived:
	...
	\begin{remark}
		...
		% TODO: Name drop the casimir above so they know what it is.
	\end{remark}
\end{example}
\begin{question}
...
This relation is true for any diagram into which each term is inserted. In-particular, it remains true after rotating all terms a quarter rotation clockwise. However it's not obvious that this is true, and various other relations have to be applied to turn the rotated version into the original relation. This suggests that perhaps the relation is a consequence of a similar relation.
