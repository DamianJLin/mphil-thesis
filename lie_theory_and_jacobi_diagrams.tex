...
\]
Since the Casimir element is the inverse of the metric, the invariance of the metric should also be reflected in a property of the Casimir.
...
\begin{proof}
	...
	\[
		...
		\sum_{i, j} \langle c_{i2}, [x, c_{j1}] \rangle c_{i1} \otimes c_{j2}.
	\]
	Since the metric and the Casimir elements are inverse, for all \(x \in \mathfrak{g}\) we have \(\sum_{i} c_{i1} \langle c_{i2}, x \rangle = x = \sum_{i} c_{i2} \langle x, c_{i1} \rangle\). Applying this to the equality above yields
...
\begin{example}
	...
	Inverting the metric gives the Casimir element
...
An algebra over an operad is an object in \(\mathcal{C}\) whose operations obey the rules defined by \(\mathcal{O}\).

Recall that an operad is in particular a collection of sets (or other objects of some symmetric monoidal category \(\mathcal{C}\)) \(\mathcal{O}(n)\) for \(n \in \mathbb{N}\), with a \(S_{n}\) action on each \(\mathcal{O}(n)\). Such an object is called an \(S\)-module, and there is the category \(S\text{-}\mathbf{mod}\) of \(S\)-modules whose morphisms are \(S\)-module morphisms: morphisms that commute with the action of \(S_{n}\).

There is also a category \(\mathbf{Oper}\) of operads. The morphisms from operads \(\mathcal{O}\) to \(\mathcal{P}\) are sequences of maps \(\mathcal{O}(n) \to \mathcal{P}(n)\), \(n \in \mathbb{N}\) that preserve the identity, preserve composition and commute with the action of \(S_{n}\).

As such, there is a natural forgetful functor \(\square: \mathbf{Oper} \to S\text{-}\mathbf{mod}\). It has a left-adjoint free operad functor \(\Lambda: S\text{-}\mathbf{mod} \to \mathbf{Oper}\). For details of this construction we refer the reader to \cite{operads-and-props}. 
...
\begin{example}
	\label{ex:lie-operad}
	With this in mind, we can describe the \textbf{Lie operad} in a simple graphical presentation in terms of generators and relations:
	\[
		\mathcal{O}_{\text{Lie}}
		\quad
		=
...
\begin{definition}
	...
	Let \(\sigma_{c} \in S_{n + 1}\) denote the cyclic permutation sending \(0 \mapsto 1, 1 \mapsto 2, \cdots, n \mapsto 0\). To be a cyclic operad, the action of \(S_{n + 1}\) must satisfy the \textbf{cyclic action condition}
...
The following definition generalises the definition of a metric Lie algebra (which is an algebra over the cyclic operad \(\mathcal{O}_{\text{Lie}}\)) to an arbitrary cyclic operad.


\begin{definition}
	\label{def:metric-algebra-over-cyclic-operad}
	For a cyclic operad \(\mathcal{O}^{\text{cyc}}\) in category \(\mathcal{C}\), a \textbf{metric algebra} over \(\mathcal{O}^{\text{cyc}}\) is a pair \((A, m)\) of a choice of object \(A \in \mathcal{C}\) which is an algebra over \(\mathcal{O}^{\text{cyc}}\) together with a symmetric morphism \(m: A \otimes A \to k\), such that (informally) using \(m\) to turn the output into an input and using its inverse \(c\) to turn the last input into an output implements the action of the cyclic operad. Formally:
	\begin{itemize}
...
\begin{theorem}[\cite{cyclic-operads-and-the-algebra-of-chord-diagrams}]
	...
	The algebra \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c})\) is isomorphic as an algebra to \(\mathcal{A}\).
\end{theorem}
