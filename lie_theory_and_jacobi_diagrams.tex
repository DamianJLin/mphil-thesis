...
\[
	...
	.
\]
Just like the antisymmetry of the bracket and the Jacobi relation, these relations make sense when the diagrams are interpreted as parts of Jacobi diagrams. There is only one type of trivalent vertex in a Jacobi diagram, and since the edges around a vertex are only considered up to cyclic order, cyclic permutation of the order is ``unseen on the level of Jacobi diagrams''.
	...
	\begin{enumerate}
		...
		\item
			...
			\[
				...
				\;.
			\]
			% TODO: (Zsuzsi) Now would be a good time to explain the arrows. An outgoing arrow at the top means the opposite as an outgoing arrow at the bottom. This is different from weight systems and needs to be said.
			% Reply: Aren't these the same as the arrows before? The flow into 1 and out of 1 dual (in the direction of composition --- so down). This is just like the thin lines in weight systems (with g and g*) except there's no longer an identification from g to g*.
...
\begin{remark}
	In \cite{cyclic-operads-and-the-algebra-of-chord-diagrams} a operad-theoretic characterisation of the primitive elements is given. Recalling from Corollary \ref{cor:primitives-equals-connected-jacobi-diagrams} that by the formula for the coproduct, the primitive elements are exactly the connected components, this also gives a definition of the coproduct without reference to connected components. The characterisation is technical, involving modular operads, and out of the scope of this text.
\end{remark}
