...
\begin{example}
	...
	Note that these identities are simply the axioms in Definition \ref{subdef:lie-algebra-object}, specialised to the category \(\mathcal{C} = \mathbf{sVect}\).
...
There is, however, a theoretical construction of Hinnich-Vaintrob \cite{cyclic-operads-and-the-algebra-of-chord-diagrams}: they construct a Lie algebra object in a symmetric monoidal category with a weight system which is isomorphic to \(\mathcal{A}\). This guarantees that every Vassiliev invariant can be constructed from applying Construction \ref{cons:lie-algebra-weight-system} to some Lie algebra object in some symmetric monoidal category. However, it doesn't give any hints as to which symmetric monoidal category one needs to work in to construct a particular Vassiliev invariant. In this sense it is useful theoretically, but not computationally.

Furthermore, the this algebra is constructed as a generalised notion of a universal enveloping algebra, applied to a Lie algebra object is some symmetric monoidal category, giving  credence to the statement hinted at by Construction \ref{cons:lie-algebra-weight-system} that ``the algebra \(\mathcal{A}\) behaves like the universal enveloping algebra of a Lie algebra''.
...
\begin{warning}
	...
	For the definitions below \(\mathcal{C}\) is not necessarily a category whose objects are sets, however we will simplify notation as if this were the case. For example we write \(x \in A\) for \(A \in \mathcal{C}\). If the objects are not necessarily sets, this should be interpreted as a morphism \(x: \mathbf{0} \to A\) where \(\mathbf{0}\) is the monoidal unit.
\end{warning}

\begin{definition}
	\label{def:operad}
	An \textbf{operad} \(\mathcal{O}\) in a symmetric monoidal category \(\mathcal{C}\) consists of:
	...
	\begin{itemize}
		...
		\item a series of \textbf{composition morphisms}:
...
The Hinich--Vaintrob construction is a Lie algebra object in a symmetric monoidal category whose universal enveloping algebra 

Recall that the goal of the Hinich--Vaintrob construction is to realise \(\mathcal{A}\) as the universal enveloping algebra (or an appropriate generalisation thereof) of a Lie algebra object in some symmetric monoidal category.

The first step is to create an category whose morphisms encodes the internal structure of \(\mathcal{A}\), i.e. the \ref{eq:AS} and \ref{eq:IHX} relations, but not yet with a choice of representation or the \ref{eq:STU} relations. An important feature of \(\mathcal{A}\) that enabled the construction of Lie algebra weight systems was that contraction of tensors corresponding to the Lie bracket or metric/casimir elements was akin to joining parts of diagrams in \(\mathcal{A}\). So a contraction-like operation would be a good start.
...
\begin{definition}
	...
	\]
	% TODO: (Zsuzsi) This might be more illuminating if instead of writing $\sigma_c$ you drew its action.
	% Reply: Yes but I think that's actually too strong of a statement. That's only necessarily what the action looks like if the cyclic structure comes from a metric --- but nothing in the definition of a cyclic structure says it should look like that.
...
\begin{definition}
	...
	\end{itemize}
\end{definition}
% TODO: (Zsuzsi) We checked this in my office at some point and I recall it's fine, once you untangle what the definition means. We can talk about it in LD.Co
% Reply: Yes, I recall we checked it! But I had a hunch we agreed that Hinnich-Vaintrob had a typo. At least I haven't managed to untangle it myself.
% Zsuzsi: If you untangle it for O=LIE, n=2, and m=(,) the Killing form, then the S_3-invariance written for the 3-cycle is the identity that ([x,y],z)=(x,[y,z]), in other words, the g-invariance of the Killing form. So this is fine, no typo. You could add this as an example or remark.
...
\end{proposition}
%TODO (Zsuzsi) Propositions should have proofs. If you don't want to write a proof, you can change this to an Example environment - same with the next proposition about Casimir.
...
%TODO (Zsuzsi) I would delete "only trivalent vertices" and just have "no univalent vertices" - since in this example there are neither

In \(\mathcal{A}\) there are no such relations. In fact, there are no chord diagrams that have only trivalent vertices and no univalent vertices. So, metric Lie algebras are the wrong thing to consider if we want to find an algebra isomorphic to \(\mathcal{A}\). Algebras having only casimirs are the correct choice.
