...
\begin{definition}
	...
	An \textbf{algebra \(A\) (in \(\mathcal{C}\)) over the prop \(\mathbf{P}\)} is tensor functor \(\alpha: \mathbf{P} \to \mathcal{C}\).
\end{definition}
...
\begin{remark}
	...
	\[
		\begin{tikzcd}
			{\mathbf{Oper}} \\
			{\mathbf{Prop}}
			\arrow[""{name=0, anchor=center, inner sep=0}, "{U}", shift left=3, from=1-1, to=2-1]
			\arrow[""{name=1, anchor=center, inner sep=0}, "{F}", shift left=3, from=2-1, to=1-1]
			\arrow["\dashv"{anchor=center}, draw=none, from=1, to=0]
		\end{tikzcd}
	\]

	The forgetful functor \(F\) takes a prop \(\mathbf{P}\) to the operad whose operations are the \(m, 1\)-ary elements of the prop (i.e. have one output):
	\[F(\mathbf{P})(m) = \mathbf{P}(m, 1).\]
	The free functor \(U\) takes an operad to the prop whose \(m, n\)-ary morphisms are tensor products of operations from the operad:
	\[U(\mathcal{O})(m, n) = \bigoplus_{f: [m] \to [n]} \bigotimes_{i = 1}^{n} \mathcal{O}(|f^{-1}(i)|).\]
\end{remark}
