...
\section{Computing some local \(W_{\mathfrak{g}}\) relations}
...
From Vogel's relations, we can determine which diagrams participate in a relation, then by computer find the corresponding tensors in \(W_{\mathfrak{g}}\), and find the relation between them in the tensor algebra of \(\mathfrak{g}\).
...
\begin{lemma}
	% Internal to \(\mathcal{A}\) (following from \textup{\ref{eq:IHX}} and \textup{\ref{eq:AS}}) is the relation
	\[
	...
	\]
	(Here the bubble may be on any of the three half-edges.)
\end{lemma}
\begin{proof}
	Apply \ref{eq:IHX} to any pair of vertices. (Treat this carefullt with \ref{eq:AS} --- don't assume that \(\tau\) is strict, as this should hold outside Lie-algebra specific weight systems.)
\end{proof}
\begin{lemma}
	For a Lie algebra \(\mathfrak{g}\), the bubble is proportional to line.
\end{lemma}
.
\end{example}
