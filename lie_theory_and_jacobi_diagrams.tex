...
We turn to the coalgebra structure of \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c})\) and the bialgebra isomorphism to \(\mathcal{A}\).

\begin{definition}
	The coproduct \(\Delta\) on \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c})\) is defined by setting elements of the form \([\cdot,\cdot] \otimes \id \otimes \xi \in \mathcal{O}_{\text{Lie}}(2) \otimes \mathcal{O}_{\text{Lie}}(1) \otimes \Gamma(\mathfrak{g})\) to be primitive, namely
	\[
		\Delta([\cdot,\cdot] \otimes \id \otimes \xi)
		=
		\left( [\cdot,\cdot] \otimes \id \otimes \xi \right) \otimes 1
		+
		1 \otimes \left( [\cdot,\cdot] \otimes \id \otimes \xi \right).
	\]
	and defining \(\Delta\) to extend to the whole of \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c}))\) by being an algebra map.
\end{definition}

\begin{lemma}
	The coproduct on \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c})\) is well-defined and compatible with the product, hence \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c})\) is a bialgebra.


	Diagramatically, such elements look like
	\[
		\def\svgscale{0.37}
		\adjustbox{valign=c}{\input{graphics/external_enveloping_algebra_lie_example_one_cc.pdf_tex}}.
	\]
\end{lemma}

\begin{proof}
	Firstly, we check that the definition above determines the coproduct on the whole of \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c})\). Indeed, \(\mathcal{O}_{\text{Lie}}\) is generated as an algebra by \([\cdot, \cdot] \in \mathcal{O}_{\text{Lie}}(2)\), elements of the form \([\cdot, \cdot] \otimes \id \otimes \xi\) for \(\xi \in \Gamma(\mathfrak{g}))\) so it suffices that \(\Delta\) is an algebra map, and it was defined as such.

	We now check that this is a well-defined operation. That \(\Delta\) is compatible with \ref{eq:AS} and \ref{eq:STU} directly follows from \ref{eq:AS} and \ref{eq:STU} in \(\Gamma(\mathfrak{g})\). We check that \(\Delta\) is compatible with the defining relation of the universal enveloping algebra \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c})\). Without loss of generality, the argument in Example \ref{ex:stu-in-external-enveloping-algebra}, but in the symmetric monoidal category \(\mathbf{E}_{c}\) rather than \(\mathbf{Vect}\).
\end{proof}

Finally, we upgrade the isomorphism in Theorem \ref{thm:universal-casimir-lie-envelope-chord-diagrams-algebra-isomorphism} to the level of bialgebras.
...
\begin{proof}
	All that remains is to check that primitives in \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c}))\) map to primitives in \(\mathcal{A}\), and vice-versa. Indeed, any primitive in \(U(\mathcal{O}_{\text{Lie}}, \mathbf{1}_{c}))\) can be written as an element of the form \([\cdot, \cdot] \otimes \id \otimes \xi\) for \(\xi \in \Gamma{\mathfrak{g}}\). Therefore it is mapped in \(\mathcal{A}\) to a diagram consisting of a single connected component, which is primitive in \(\mathcal{A}\).

	Conversely, diagrams consisting of single connected components in \(\mathcal{A}\) have a vertex that is 
\end{proof}

% TODO: Wait, but how does this fare with primitives with more than one connection to the skeleton? Hmmmm.

Theorem \ref{thm:universal-casimir-lie-envelope-chord-diagrams-algebra-isomorphism} is a formalisation of the statement that ``the algebra \(\mathcal{A}\) behaves like the universal enveloping algebra of a Lie algebra''. In the next chapter we look at a generalisation of this statement in the context of welded knots.
