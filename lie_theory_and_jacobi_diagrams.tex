...
If a monoidal category \(\mathcal{C}\) is additionally additive, we define Lie algebra objects internal to \(\mathcal{C}\).

\begin{definitions}
	\label{def:internal-to-additive-monoidal-category}
	\begin{enumerate}
		\item A \textbf{Lie algebra object} in an additive symmetric monoidal category \(\mathcal{C}\) is an object \(L\) equipped with a bracket morphism \(\beta : \mathcal{C} \otimes \mathcal{C} \to \mathcal{C}\) such that
...
\begin{warning} %TODO (Zsuzsi) Is this a warning, or just conventiuon?
	For all of the below, we assume categories are \(k\)-linear, where \(k\) is a field of characteristic zero.
\end{warning}
