...
\begin{proposition}
	The connected sum \(\#: \mathcal{K}_{0} \otimes \mathcal{K}_{0} \to \mathcal{K}_{0}\) forms a well-defined operation. It does not matter where along either knot the small arc was removed, the results are ambient-isotopic.
\end{proposition}

\begin{proof}
	We exhibit an ambient isotopy starting at \(K_{1} \# K_{2}\) where the small arc is removed from \(K_{1}\) as in the example above. The part of the connected sum coming from \(K_{2}\) is shrunk by ambient isotopy. Since it can be shrunk arbitrarily small, let it be shrunk to lie within a small tubular neighbourhood of \(K_{1}\).
	\[
...
\begin{proposition}
	There is a descending filtration on the space \(\mathcal{K}\)
	\[\mathcal{K}_{0} \supset \delta^{1}(\mathcal{K}_{1}) \supset \delta^{2}(\mathcal{K}_{2}) \supset \cdots\]
	giving \(\mathcal{K}\) with the connected sum as multiplication the structure of a filtered algebra.
\end{proposition}
...
\begin{proof}
	...
	For the former, if \(k \otimes \ell \in \mathcal{K}_{i} \otimes \mathcal{K}_{j}\), then \(k\) and \(\ell\) are images of singular knots under resolving \(i\) and \(j\) double points respectively.
\end{proof}

\begin{mdframed}
	Need to add an additional topological move for ambient isotopy on singular knots that allows moving a string link through a double-point (true after resolving). Need to make a remark here that well-definedness of the singular knot connected sum was exactly the point of having this move.
\end{mdframed}

\begin{proposition}
	The operation \(\#: \mathcal{K} \otimes \mathcal{K} \to \mathcal{K}\) makes \(\mathcal{K}\) into a filtered algebra, with the grading given by the number of singular points.
\end{proposition}
