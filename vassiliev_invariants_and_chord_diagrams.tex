...
\begin{definitions}
	\begin{enumerate}
		\item The \textbf{associated graded} algebra of a filtered algebra \(A\) is the algebra
			\[\gr A = \bigoplus_{m = 0}^{\infty} A_{m} / A_{m + 1}.\]
		\item There is a category of filtered algebras and filtration-respecting maps, and there is a category of graded algebras and grading-respecting maps. The \textbf{associated graded} map of a filtration-respecting map \(f: A \to B\) is the grading-respecting map \({\gr f : \gr A \to \gr B}\) defined by taking a representative \(a_{m}\) to \(f(a_{m})\) then projecting onto \(B_{m} / B_{m + 1}\),
...
The fundamental theorem of Vassiliev invariants is that it allows us to establish a relationship between knots and chord diagrams, or equivalently, Vassiliev invariants and weight systems. It's not a-priori obvious how Theorem \ref{thm:fundamental-theorem-of-vassiliev-invariants} does this, but is apparent in the following equivalent reformulation.
...
The part due to Vassiliev is the easy part.

\begin{proofof}{Vassiliev's part of Theorem \ref{thm:fundamental-theorem-restated}}
	\begin{mdframed}
		We start with the map \(a \in \mathcal{A}_{m} \to \mathcal{K}_{m} / \mathcal{K}_{m + 1}\). The map is given by choosing a singular knot in \(\singularknots_{m}\) whose chord diagram is \(a\), then resolving it to an element of \(\mathcal{K}_{m}\), and projecting that into the quotient \(\mathcal{K}_{m} / \mathcal{K}_{m + 1}\).
		...
		Make the argument in the dual case.
	\end{mdframed}
\end{proofof}
...
\begin{theorem}[Kontsevich Integral]
	There is an invariant of knots, called the Kontsevich integral \(Z: \mathcal{K} \to \widehat{\mathcal{A}}\) with the following property. If \(k \in \mathcal{K}\) such that \(a \in \mathcal{A} \to [k] \in \mathcal{K}_{m} / \mathcal{K}_{m + 1}\), then
	\[Z(k) = a + \cdots\]
\end{theorem}
