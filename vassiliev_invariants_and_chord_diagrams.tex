...
\begin{proposition}
	Suppose \(P\) is an integrable \(m\)-singular invariant with integral \(Q\). Let \(Q_{0}\) differ from \(Q\) by a %TODO(Zsuzsi) linear
    function on chord diagrams, that is
...
\begin{proof}
	The derivative of a %TODO (Zsuzsi) linear
    function of chord diagrams is zero, as two knots which differ by a crossing change have the same chord diagram. 
    %TODO (Zsuzsi) This could be more precise. If two m-singular knots differ by a crossing change, then their difference is (m+1)-singular and hence zero in D_m -- or something to this effect but in your language
    Hence \(Q\) and \(Q_{0}\) have the same derivative: \(P\).
\end{proof}

\begin{remark}
	We defined a constant of integration as a set of objects (e.g. \(c \in \mathcal{D}_{m}\)) %TODO (Zsuzsi) I found this a bit confusing. Integration constants were only defined in the general context, so it would be helpful to explain how that translates. Then again I started reading here after a few weeks' break, so maybe it would be easier in one sitting.
    but perhaps it would have been more accurate to define it as a functional (e.g. \(q \in \mathcal{D}_{m}^{\ast}\)). After all the constant of integration on the real line is ``\(+ \ C\)'' rather than the set \(\{\ast\}\) with one object. %TODO (Zsuzsi) Why is the analogue a set with one object?
    There is no real risk of confusion, so let us be slightly loose and use the terminology for either. %TODO (Zsuzsi) I, for one, am confused:).
\end{remark}
...
If we take the relations for a knot invariant to be integrable %TODO (Zsuzsi) I first read this as "If we assume that the relations for a knot invariant are integrable"... Perhaps rephrase, like "the relations that a knot invariant needs to respect in order to be integrable".
and project %TODO (Zsuzsi) What does 'project' mean here? I would state the specific map you are using.
them into the space of chord diagrams, we get the following relations.
...
\begin{definitions}
	...
	\begin{enumerate}
		...
		\item The \textbf{one-term relation} is the relation
			...
			\end{equation}
	\end{enumerate}
	Just like \ref{eq:T4Tstar} and \ref{eq:T1Tstar}, \ref{eq:4Tstar} and \ref{eq:1Tstar} are not individual relations, but kinds %TODO (Zsuzsi) Classes of relations?
    of relations. There's one \ref{eq:4Tstar} relation for all ways of placing other chords in-between the three far-apart chord ends, 
    ...
    %TODO (Zsuzsi) But identically in all four
    There's a \ref{eq:1Tstar} relation for all ways of placing chord diagrams between either of the 
    %TODO (Zsuzsi) Here too, I'd say along the long arc, or dotted arc. 
    two far-apart chord ends. In other words, any chord %TODO (Zsuzsi) diagram 
    with an `isolated chord' that doesn't cross any other chords in the diagram is killed by a \ref{eq:1Tstar} relation.
\end{definitions}
...
We return now to the secondary (and higher) obstructions. A general integral of an \(m\)-singular \(P\) %TODO (Zsuzsi) Is $P$ an invariant of m-singular knots?
is of the form
\[Q + q \circ \sigma.\]
...
\begin{theorem}[Fundamental theorem of Vassiliev invariants]
	...
	Let \(v\) be an invariant of \(m\)-singular knots satsifying \textup{\ref{eq:T4Tstar}}, \textup{\ref{eq:T1Tstar}} and the additional condition that \(\delta^{k} v = 0\). Then \(v\) integrates \(m\) times into a genuine knot invariant (which is a Vassiliev invariant). %TODO (Zsuzsi) of type m+k?
\end{theorem}
...
%TODO (Zsuzsi) This purely a style comment: since you're talking about Dror's philosophical stance, it would be nice to do so in active voice and name him.
To quote their introduction: ``Always the method is indirect and very complicated, and/or some a-priori unnatural choices have to be made''. To summarise their philosophy:

\begin{remark}
	We have the implication Conjecture \ref{conj:strong-fundamental-theorem} \(\implies\) Theorem \ref{thm:fundamental-theorem-of-vassiliev-invariants}, and this is actually realised in the theory of braids. It is mysterious that the fate of the slightly stronger conjecture which comes from taking the natural topological approach to the fundamental theorem still remains unknown, and that there are grievances to be had with all known proofs.%TODO (ZSuzsi) of the theorem
\end{remark}

In Section \ref{sec:the-fundamental-theorem-of-vassiliev-invariants} we will look at equivalent formulation of the fundamental theorem. %TODO (Zsuzsi) Do you want to foreshadow why/ what the advantage of the different formulation is?
...
\begin{definitions}
	\begin{enumerate}
		\item The \textbf{space of knots}, denoted \(\mathcal{K}\), is the vector space spanned (over \(\mathbb{Q}\)) by non-singular knots. Equivalently, \(\mathcal{K} = \singularknots_{0}\). %TODO (zsuzsi) Do something about the alignment here? No need for paretheses for "over Q".
		\item The space of knots is equipped with the \textbf{singular knot filtration}
			...
			where the \(i\)th filtered component \(\mathcal{K}_{i}\) is the span of resolutions of singular knots with \(i\) double points, equivalently the image of \(\delta^{i}\); \(\mathcal{K}_{i} = \delta^{i}(\singularknots_{i})\). %TODO (Zsuzsi) I'd separate adjacent formulas with a couple of word like "that is," rather than semicolon.
	\end{enumerate}
\end{definitions}
...
\begin{proof}
	A filtration of vector spaces is uninteresting. %TODO (Zsuzsi) Philosphical remarks don't belong in a Proof environment :)
    The only thing to check is that if \(i < j\), then \(\mathcal{K}_{i} \supset \mathcal{K}_{j}\). If \(k \in \mathcal{K}_{j}\), then \(k = \delta^{j}(k^{\bullet})\) for some \(k^{\bullet}\) in \(\mathcal{K}_{j}\). But then \(k = \delta^{j}(k^{\bullet}) = \delta^{i}\delta^{j - i}(k^{\bullet})\), so \(k \in \delta^{i}(\singularknots_{i})\).
\end{proof}
...
This %TODO (Zsuzsi) "This" -> "The connected sum of knots"
is not a-priori well-defined, as we have not specified where along either \(k_{1}\) or \(k_{2}\) the small arc is to be removed. %TODO (ZSuzsi) Or along what path they should be connected.
...
\begin{proposition}
	...
	The connected sum \(\connect : \mathcal{K} \otimes \mathcal{K} \to \mathcal{K}\) forms %TODO (Zsuzsi) "forms" -> "is"
    a well-defined operation. It does not matter where along either knot the small arc was removed, the results are ambient-isotopic. %Todo (Zsuzsi) "Up to ambient isotopy, the connected sum of two knots is independent of the choice of where and along what path the connection was performed."
\end{proposition}
...
\begin{proof}
	...
	\]
	The above argument works for any choice of small arc removed along \(k_{1}\), and the same argument with the roles of \(k_{1}\) and \(k_{2}\) reversed completes the proof. %TODO (Zsuzsi) This also shows that the path along which you connect doesn't matter, because you can "pull" tiny k_2 along it until it's within the tubular nbhd of k_1.
\end{proof}

\begin{proposition}
	The connected sum respects the descending filtration:
	\[K_{i} \connect K_{j} \subset K_{i + j}.\]
	That is, the connected sum makes \((\mathcal{K}, \connect)\) into an ascending filtered algebra. %TODO (Zsuzsi) Descending?
\end{proposition}
...
\begin{proof}
	We will prove that the connected sums, given any two choices of connection locations, are equal modulo \ref{eq:4T}.
...
\begin{definition}
	...
	The graded bialgebra \(\mathcal{W}\) is the graded bialgebraic dual of the graded bialgebra \(\mathcal{A}\).
\end{definition}

In fact, considering a graded object as a filtered object with the naturally induced filtration, this coincides with the filtered bialgebraic dual.

\begin{proof}
	Weight systems are linear functionals on chord diagrams, \(\mathcal{D}\) that vanish on \ref{eq:4Tstar} and \ref{eq:1Tstar}. This is the same as linear functionals on \(\mathcal{A} = \mathcal{D}/\ref{eq:4T}, \ref{eq:1T}\).

	The rest of the proof, (i.e. that the product in \(\mathcal{W}\) is dual to the coproduct in \(\mathcal{A}\) and vice versa) follows exactly as in the proof sketch of the duality of bialgebras \(\mathcal{K}\) and \(\mathcal{V}\).
\end{proof}
...
It is actually only the second part of this theorem, due to Kontesevich that is equivalent to Theorem \ref{thm:fundamental-theorem-of-vassiliev-invariants}. The first part, due to Vassiliev, is additional (so in reality, this `reformulation' is stronger), but considered ``easy'' relative to the second part. If the goal is to describe \(\mathcal{K}\) by \(\mathcal{A}\), it states that the relations in \(\mathcal{A}\), \ref{eq:4T} and \ref{eq:1T} are truly compatible with \(\mathcal{K}_{n}/\mathcal{K}_{n + 1}\). Indeed this was the point of Section \ref{sec:the-stratification-on-the-space-of-knots-and-integration}, so most of the work is already done.
...
\begin{definition}
	...
	In other words this is the degree-completion of the graded algebra \(\gr A\). Note that \(\widehat{A}\) is only a filtered algebra, not a graded one, as its elements can have infinitely many terms non-zero, so it doesn't decompose as a direct sum.
\end{definition}
