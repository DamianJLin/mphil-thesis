...
Connected, commutative, cocommutative graded bialgebras of finite type are very rigid structures. In-particular, a classical strucutral theorem applies, and the Hopf algebra
%TODO (Zsuzsi) Switched to calling it a Hopf algebra
can be understood in terms of its primitive elements.
...
\begin{proof}
 %TODO (ZSuzsi) Proof of what? I think you need to take that statement out of the Definition into a Proposition environment.
...
It is actually only the second part of this theorem, due to Kontesevich that is equivalent to Theorem \ref{thm:fundamental-theorem-of-vassiliev-invariants}. The first part, due to Vassiliev, is additional (so in reality, this `reformulation' is stronger), but considered ``easy'' relative to the second part. 
%TODO (Zsuzsi) I would rearrange this sentence so that it's clear what easy refers to
If the goal is to describe \(\mathcal{K}\) by \(\mathcal{A}\),
%TODO (Zsuzsi) via A? using A?
it 
%TODO (Zsuzsi) What is 'it' referring to here?
states that the relations in \(\mathcal{A}\), \ref{eq:4T} and \ref{eq:1T} are truly compatible with \(\mathcal{K}_{n}/\mathcal{K}_{n + 1}\). Indeed this was the point of Section \ref{sec:the-stratification-on-the-space-of-knots-and-integration}, so most of the work is already done.
% TODO: Find reference for Vassiliev
% TODO: Rephrase this as not to call Vassiliev's work ``easy''. (Zsuzsi): Only 'easy' compared to Kontsevich's Fields medal... happens to the best of us;)
