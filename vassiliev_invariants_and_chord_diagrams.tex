...
\begin{definition}
	We define \(\mathcal{A}\), the \textbf{space of chord diagrams} as
	\[\mathcal{A} = \mathcal{D} / \textup{\ref{eq:co4T}, \ref{eq:co1T}}.\]
\end{definition}

\begin{warning}
	Both \(\mathcal{A}\) and \(\mathcal{D}\) are known as the space of chord diagrams. From now on when we refer to a chord diagram, we mean an element of \(\mathcal{A}\) unless otherwise specified.
\end{warning}
