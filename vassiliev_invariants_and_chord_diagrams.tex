...
\begin{definition}
	The \textbf{primitive elements}, \(\mathcal{P}(A)\) of a coalgebra \(A\) are the elements satisfying
	\[\Delta(x) = 1 \otimes x + x \otimes 1.\]
\end{definition}

% TODO: Define connected.
The algebra \(\mathcal{A}\) is a connected, graded algebra of finite type. Such a structure is sometimes called a \textbf{Hopf algebra}. Furthermore, it is cocommutative and commutative, and for such algebras there is an important classical theorem about the structures of such objects due to Milnor and Moore \cite{on-the-structure-of-hopf-algebras}.
...
\begin{remark}
	In modernity, Hopf algebras are defined as not-necessarily-graded bialgebras with an additional operation called the antipode. This definition will not be so useful for our purposes, but note that connected, graded algebras of finite type can always be given an antipode, and so are examples of what are today called Hopf algebras.
\end{remark}

The primitive elements of \(\mathcal{A}\) will come in handy in Chapter \ref{ch:lie-theory-and-jacobi-diagrams} when we make connections to Lie algebras.
