...
\begin{definitions}
	...
	\begin{enumerate}
		...
		an oriented circle with a distinguished set of \(m\) unordered pairs of points, considered up to orientation-preserving diffeomorphism of the circle. In figures, chords are drawn between each pair of points. The vector space spanned by chord diagrams of order \(m\) is denoted \(\mathcal{D}_{m}\), so \(\mathcal{D}_{m} = C\singularknots_{m}\).
...
\begin{definitions}
	...
	\begin{enumerate}
		...
		\item The \textbf{one-term relation} (only relevant in the unframed case) is the relation
			\begin{equation}
				\label{eq:1Tstar}
				\tag{\onetermstar}
			...
			\end{equation}
	\end{enumerate}
	Just like \ref{eq:T4Tstar}, \ref{eq:4Tstar} is not an individual relation, but a class of relations. There's one \ref{eq:4Tstar} relation for all ways of placing other chords in-between the three far-apart chord ends, 
    ...
    two far-apart chord ends. In the unframed case, any chord diagram with an `isolated chord' that doesn't cross any other chords in the diagram is killed by a \ref{eq:1Tstar} relation.
\end{definitions}
...
% TODO: The partials below should be deltas
...
\begin{proofof}{Theorem \ref{thm:fundamental-theorem-restated} (dual)}
	...
	\Theta(k) =
		\begin{cases}
			...
			\\
		\end{cases},
	\]%TODO (Zsuzsi) These cases use the opposite notation from usual: usually we write the value first, folowed by "if [condition]". It reads a bit confusing, though it became cleare once I realised how to read it. 
