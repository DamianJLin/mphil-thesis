...
% (Damian) Tangential points are excluded, but they don't hold the same status as the above which lead to 4T and 1T. So I am happy with this. (You can delete this when you see it if I've convinced you).
...
% NOTE: (Damian) No, as in we have motivated the DIFF relation but where does the diff* relation come from? This philosophy. I don't think I will talk in that level of detail in the introduction.
...
\begin{remark}
	The derivative operation and the \ref{eq:diff} relation are dual to the boundary operation and the \ref{eq:codiff} relation. For example, a differentiable invariant of knots is the same as a invariant of knots in \(\mathcal{K}_{m}\).
	% TODO: (Zsuzsi) This might deserve to be deminstrated on a small example.
	% TODO: (Zsuzsi) Worth noting that any invariant of actual (non-singular) knots is differentiable.
\end{remark}
...
% TODO: (Zsuzsi) Insert an intro sentence so (a) and (b) are aligned
% NOTE: (Damian) I actually prefer it this way -- I think the (x) makes it easy enough to see, and I dislike being forced to come up with an intro sentence where one doesn't belong.
...
\begin{definitions}
	...
	\item The \textbf{order} of a Vassiliev invariant \(V\) it the highest \(m\) such that \(V\) is a Vassiliev invariant of order \(m\). (That is, the order of a Vassiliev invariant is the most double points a knot \(K\) can have without \(V(K)\) having to vanish).
	\end{enumerate}
	% TODO: (Zsuzsi) Highest or lowest?
	% NOTE: (Damian) Indeed, highest.
\end{definitions}
...
Just like with the \ref{eq:diff} and \ref{eq:codiff} relations, instead of studying functions on chord diagrams satisfying \ref{eq:4T} and \ref{eq:1T}, we prefer to study functions on spaces quotiented by relations on the objects.
...
\begin{definition}
	...
	\[\mathcal{A} = \mathcal{D} / \textup{\ref{eq:co4T}}.\]
\end{definition}
...
\begin{mdframed}
	...
	\begin{itemize}
		\item Begin with the connected sum on plain knots (don't talk about modding by 1T 4T yet).
		\item This induces a connected sum on singular knots (define via commutative diagram).
		\item This nearly induces a conntected sum on chord diagrams... except need to mod out by 4T and 1T which we were gonna do anyway.
	\end{itemize}
\end{mdframed}
