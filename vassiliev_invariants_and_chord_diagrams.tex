...
\begin{proof}[Equivalence of Theorems \ref{thm:fundamental-theorem-of-vassiliev-invariants} and \ref{thm:fundamental-theorem-restated}]
	For the forward direction, suppose Theorem \ref{thm:fundamental-theorem-of-vassiliev-invariants} holds. This states that every invariant \(v\) of \(m\)-singular knots satisfying \ref{eq:T4Tstar} and \ref{eq:T1Tstar} and further that \(\delta v = 0\), then \(v\) integrates to an invariant of \(0\)-singular knots.

	First we prove that the Kontsevich part of the fundamental theorem holds. Let \(W\) be a weight system of order \(m\). Then \(W\) defines an invariant \(W'\) of \(m\)-singular knots by \(W'(k) = W(\sigma(k))\). The derivative of \(W'\) is zero. Indeed, 
	\[\delta W'(k) = W(\sigma(k^{+})) - W(\sigma(k^{-}))\]
	...
	The Vassiliev part of the theorem is independent of the original version and was the section of Section \ref{sec:the-stratification-on-the-space-of-knots-and-integration}.

	Now for the reverse direction, suppose Theorem \ref{thm:fundamental-theorem-restated} holds. Take a \(m\)-singular knot invariant \(v\) satisfying \ref{eq:4Tstar} and \ref{eq:1Tstar}, and that \(\delta v = 0\). This defines a weight system \(W_{v}\), which by the Kontsevich part of Theorem \ref{thm:fundamental-theorem-restated} gives an element of \(\mathcal{V}_{m} / \mathcal{V}_{m - 1}\).

	\begin{shaded}
		Check that this is an integral.
	\end{shaded}
\end{proof}
