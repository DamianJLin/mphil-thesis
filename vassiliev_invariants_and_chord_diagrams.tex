...
% TODO: (Zsuzsi) One way to approach this would be to say that the set of knots forms a commutative monoid, and $\mathcal{K}$ is its monoid algebra.
% Just like the group algebra of a group is made into a bialgbera by a natural coproduct which makes the elements of the group "group-like", the same is true for the monoid algebra of a monoid.
...
% TODO: (Damian) Let's talk about this.
...
% NOTE: (ZSuzsi) I don't think that's true, it needs to be stated for just the Vassiliev part, which is a bit more cumbersome to write.
% TODO: Ask Zsuzsi the above.
...
\begin{proof}
	...
	\begin{shaded}
		For the unframed case note that the connected sum of a chord diagram containing a \ref{eq:1T} chord relation still contains a \ref{eq:1T} chord.

		[Shouldn't the grading be \(-\infty\) then?]
	\end{shaded}
\end{proof}
...
\begin{proofof}{Theorem \ref{thm:fundamental-theorem-restated} (Vassiliev)}
	% Possibly use: The aim is to define a map \(\mathcal{A}_{m} \to \mathcal{K}_{m} / \mathcal{K}_{m + 1}\). Let us start with a map \(\mathcal{D}_{m} \to \mathcal{K}_{m} / \mathcal{K}_{m + 1}\).
	For the map \(\mathcal{A}_{m} \to \mathcal{K}_{m} / \mathcal{K}_{m + 1}\) we take the following.
	%TODO (Zsuzsi) I would rephrase this sentence. Perhaos you should first define the map ton $D_m$, and then descend to $A_m$ (which is what you practically do).
	...
	This is well-defined since any other \({\ell^{\bullet}}\) that resolves to \(k\) also has chord diagram \(a\).
	Any two \(m\)-singular knots with the same chord diagram differ by some crossing changes, in particular this applies to \(k^{\bullet}\) and \({\ell^{\bullet}}\). But if \(k^{\bullet}\) and \({\ell^{\bullet}}\) differ by a crossing change, then \(\partial^{m}(k^{\bullet})\) and \(\partial^{m}({\ell^{\bullet}})\) differ by an element of \(\mathcal{K}_{m + 1}\), so \([\partial^{m}(k^{\bullet})] = [\partial^{m}({\ell^{\bullet}})]\). This argument that any \(k^{\bullet} \in \singularknots_{m}\) can be chosen to represent \([\partial^{m}(k^{\bullet})]\) as long as it has chord diagram \(a\) will be used again below, and we refer to it as the crossing-change argument.
...
\end{lemma} %TODO (Zsuzsi) I would state this only precisely, and then note that you'll drop the projection in formulas and write $W(Z(k))$.

\begin{proof}
	The map \(W \circ Z\) is clearly an invariant, since \(Z\) is an invariant. It's Vassiliev since on linear combinations \(k \in \mathcal{K}_{m + 1}\), \(Z(k)\) has nothing in degree \(m\), 
    %TODO "$Z(k)$ is zero in derees up to m due to the universal property, so projecting to $\mathcal{A}_m$ gives zero"
    so composing with a weight system of degree \(m\) gives zero. 
\end{proof}
