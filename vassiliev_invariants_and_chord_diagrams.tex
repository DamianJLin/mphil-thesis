...
\begin{mdframed}
	...
	In view of this point, we spend Section \ref{sec:singular-knots} looking at a the stratification of the space of knots. This leads to a beautiful (and fruitful) classical analogy which we will explored in Section \ref{sec:integration-and-the-fundamental-theorem} and throughout this chapter.

	In Section \ref{sec:the-bialgebras-of-knots}, with the context in mind, we introduce the main players in this theory.
\end{mdframed}
...
In Section \ref{sec:the-kontsevich-integral} we will look at equivalent formulation of the fundamental theorem.

\section{The bialgebra of knots}
\label{sec:the-bialgebras-of-knots}

\begin{mdframed}
	Some planning notes for this chapter:
...
Speaking broadly, the aim of Vassiliev theory is to study the space of knots via the space of chord diagrams, using the information of the stratification of knots introduced in the first two sections. Knots and chord diagrams are not just vector spaces; they have some further structure. In this section, we synthesise all of this information into two algebraic structures.
...
\begin{definitions}
	\begin{enumerate}
		...
		\item The space of knots is equipped with the \textbf{singular knot filtration}
			\[\mathcal{K} \supset \mathcal{K}_{1} \supset \mathcal{K}_{2} \supset \cdots\]
			where the \(i\)th filtered component is the span of resolutions of singular knots with \(i\) double points, equivalently the image of \(\delta^{i}\); \(\mathcal{K}_{i} = \delta^{i}(\singularknots_{i})\).
	\end{enumerate}
\end{definitions}
...
\begin{proof}
	A filtration of vector spaces is uninteresting. The only thing to check is that \(\mathcal{K}_{i} \supset \mathcal{K}_{j}\) if \(i < j\). If \(k \in \mathcal{K}_{j}\), then \(k = \delta^{j}(k^{\bullet})\) for some \(k^{\bullet}\) in \(\mathcal{K}_{j}\). But then \(k = \delta^{j}(k^{\bullet}) = \delta^{i}\delta^{j - i}(k^{\bullet})\), so \(k \in \delta^{i}(\singularknots_{i})\).
\end{proof}
...
This is not a-priori well-defined. We have not specified where along either \(k_{1}\) or \(k_{2}\) the small arc is to be removed. However, by a classical knot-theoretic argument, the result is independent of the choice.
...
\begin{proposition}
	The connected sum makes \((\mathcal{K}, \hash)\) into a filtered algebra. That is, the connected sum respects the filtration,
...
\begin{proposition}
	The connected sum and coproduct make \((\mathcal{K}, \hash, \Delta)\) into a bialgebra.
\end{proposition}
...
\begin{proof}
	...
	\begin{align*}
		...
		& = (k \otimes k) \hash (\ell \otimes \ell) \\
		& = \Delta(k) \hash \Delta(\ell)
	\end{align*}
...
\begin{proposition}
	...
	With the singular knot filtration, \(\mathcal{K}\) is a filtered bialgebra. That is, the coproduct \(\Delta\) also respects the filtration,
	\[\Delta(\mathcal{K}_{j}) \subset \sum_{i = 0}^{j} \mathcal{K}_{i} \otimes \mathcal{K}_{j - i}.\]
\end{proposition}
% TODO: Make comment that the dual of this theorem is analagous to the Leibniz formula, and how this relates to the Vassiliev invariants polynomials analogy.

We give a proof, due to Willerton \cite{vassiliev-invariants-and-the-hopf-algebra-of-chord-diagrams} which follows directly from his Lemma \ref{lem:co-leibniz-formula-for-knots}. The lemma gives a formula for the coproduct of an element of \(\mathcal{K}_{n}\) in terms of a singular knot that resolves to it by looking at the \(2^{n}\) ways of resolving some of singular points in one cofactor and the rest in the other. If \(I\) is a subset of the singular points of a singular knot, let \(\delta^{I}\) be the operator that resolves the singular points \(I\). Let \(\mu^{I}\) be the operator that averages singular points in \(I\), where averaging a singular point is sending
...
\begin{proof}
	...
	\begin{align*}
		...
		& \frac{1}{2} \sum_{\mathclap{x \notin I \subset J}}
			...
			\Big)
	\end{align*}
	but since \(\{I \mid I \subset J, x \notin I\}\) is in bijection with \(\{I \smallsetminus \{x\} \mid I \subset J, x \in I\}\), in each of the above sums, the corresponding terms have the same indices. Hence, the first and last terms in each sum combine, and the second and third terms cancel out to give
	\[
		\sum_{\mathclap{x \in I \subset J}}
			\mu^{\bar{I}}\delta^{I \smallsetminus \{x\}}(k^{\bullet +})
			\otimes
...
\begin{remark}
	\label{rem:non-vassiliev-invariants}
	Not all knot invariants respect the singular knot filtration, but the point of the Vassiliev invariants is that they're the ones that are natural with respect to the singular knot filtration.
\end{remark}

To illustrate this point, let's look at the dual filtered bialgebra of \(\mathcal{K}\).

\begin{definition}
	The \textbf{filtered bialgebra of Vassiliev invariants} is the set of Vassiliev invariants, with the (increasing) filtration given by the degree of the Vassiliev invariant, the product given by multiplication pointwise
	\[V_{1}V_{2}(k) = V_{1}(k)V_{2}(k),\]
	and the coproduct \(\eta\) given by
	\[\eta(V)(k_{1} \otimes k_{2}) = V(k_{1} \hash k_{2}).\]
\end{definition}

\begin{proposition}
	The filtered bialgebraic dual of the (descending) filtered bialgebra \(\mathcal{K}\) is the (ascending) filtered bialgebra of Vassiliev invariants.
\end{proposition}
...
\begin{example}
	\begin{mdframed}
		In view of Remark \ref{rem:non-vassiliev-invariants}, let's define a knot invariant. (Define \(f_{3_{1}}\).)
	\end{mdframed}
\end{example}

\begin{remark}
	Let's continue the polynomial analogy with an observation due to Willerton in \cite{vassiliev-invariants-and-the-hopf-algebra-of-chord-diagrams}. Taking the dual statement to Lemma \ref{lem:co-leibniz-formula-for-knots}, we get a theorem analogous to the Leibniz theorem for functions of multiple variables. The statement that Vassiliev invariants are a filtered algebra then follows from the definition of Vassiliev invariants and the lemma. The same argument in multivariable calculus uses the Leibniz theorem to prove that polynomials are a filtered vector space.
\end{remark}

\section{The Hopf algebra of chord diagrams}
\label{the-hopf-algebra-of-chord-diagrams}
...
In Section \ref{sec:integration-and-the-fundamental-theorem} we saw how functions on chord diagrams specify Vassiliev invariants, so long as the functions satisfy \(\ref{eq:4T}\) and \(\ref{eq:1T}\). We can instead encode this directly into the algebra of chord diagrams by the following relations.
...
\begin{proof}
	\begin{mdframed}
		The connected sum of a chord diagram of order \(i\) and a chord diagram of order \(j\) is a chord diagram of order \(i + j\).
	\end{mdframed}
\end{proof}

\begin{mdframed}
	The coproduct on chord diagrams (will this be relevant if we use the other coproduct later anyway?). And the antipode I suppose.
\end{mdframed}

\begin{proposition}
	The graded Hopf-algebraic dual of \(\mathcal{A}\) is the the Hopf algebra \(\mathcal{W}\) of weight systems.
\end{proposition}
...
It is actually only the second part of this theorem, due to Kontesevich that is equivalent to Theorem \ref{thm:fundamental-theorem-of-vassiliev-invariants}. The first part, due to Vassiliev, is additional (so in reality, this `reformulation' is stronger), but considered ``easy'' relative to the second part. If the goal is to describe \(\mathcal{K}\) by \(\mathcal{A}\), it states that the relations in \(\mathcal{A}\), \ref{eq:co4T} and \ref{eq:co1T} are truly compatible with \(\mathcal{K}_{n}/\mathcal{K}_{n + 1}\). Indeed this was the point of Section \ref{sec:integration-and-the-fundamental-theorem}, so most of the work is already done.
...
\begin{proofof}{Theorem \ref{thm:fundamental-theorem-restated} (Vassiliev)}
	We start by defining the map \(a \in \mathcal{A}_{m} \to \mathcal{K}_{m} / \mathcal{K}_{m + 1}\). The map is given by choosing a singular knot \(k^{\bullet} \in \singularknots_{m}\) whose chord diagram is \(a\), then resolving it to an element of \(k \in \mathcal{K}_{m}\), and projecting that into the quotient \(\mathcal{K}_{m} / \mathcal{K}_{m + 1}\). This is well-defined since any other \({k^{\bullet}}'\) also has chord diagram \(a\). So by allowing crossing changes in \(\singularknots_{m}\), \(k^{\bullet}\) and \({k^{\bullet}}'\) may be made equivalent. In the presence of \(n\) singular points, the \(\delta\)-images of singular knots in \(\singularknots_{n}\) are in the \(\delta\)-image of \(\singularknots_{n + 1}\), so equivalent in the quotient.

	\begin{mdframed}
		Insert relevant figures.
	\end{mdframed}

	Recalling that \(\mathcal{A}_{m} = \mathcal{D}_{m} / \ref{eq:co1T}, \ref{eq:co4T}\), we need to show that the map is well-defined with respect to those relations. Indeed \ref{eq:co1T} is in the kernel, as any singular knot with a \ref{eq:co1T} singular point resolves to a difference of two of the same knots when that singular point is resolved.

	\ref{eq:co4T} is also in the kernel as it comes from a closed singular isotopy in the strata. When resolving such a closed isotopy, things always cancel out.

	\begin{mdframed}
		Insert figures and make an argument about the dual case.
	\end{mdframed}
	\phantom{.}
\end{proofof}

Let's turn to the other part of the fundamental theorem, with the following definition.
...
\begin{theorem}[Kontsevich]
	\label{thm:kontsevich-integral-degree-property}
	There is an invariant of knots, \(Z: \mathcal{K} \to \widehat{\mathcal{A}}\), called the Kontsevich integral, with the following property. If \(k \in \mathcal{K}_{m}\) is a linear combination of knots with \(k = \delta^{m}(k^{\bullet})\) and \(k^{\bullet}\) has chord diagram \(a \in \mathcal{A}_{m}\), then
	\[Z(k) = a + \text{higher degree terms.}\]
\end{theorem}

\begin{proofof}{Theorem \ref{thm:fundamental-theorem-restated} (Kontsevich)}
	Take the map \(k \in \mathcal{K}_{m} \to \mathcal{A}_{m}\) coming from killing the higher degree terms in the Kontsevich integral, and taking the lowest order non-zero chord diagram. This factors through the quotient to a map \(\mathcal{K}_{m}/\mathcal{K}_{m + 1} \to \mathcal{A}_{m}\) since by Theorem \ref{thm:kontsevich-integral-degree-property}, any additional \(k' \in \mathcal{K}_{m + 1}\) contributes only higher degree terms, which get killed. It is easy to see that the two maps are inverses.

	\begin{mdframed}
		Make argument about the dual case, which is taking the kontsevich integral then composing with a weight system gives a Vassiliev invariant.
	\end{mdframed}
\end{proofof}

\[\star \star \star\] % NOTE: Stuff beyond here is an old version.

\begin{definition}
	There is a category of filtered algebras and filtration-respecting maps, and there is a category of graded algebras and grading-respecting maps. The \textbf{associated graded} map of a filtration-respecting map \(f: A \to B\) is the grading-respecting map \({\gr f : \gr A \to \gr B}\) defined by taking a representative \(a_{m}\) to \(f(a_{m})\) then projecting onto \(B_{m} / B_{m + 1}\),
		\[\gr f: a_{m} + A_{m + 1} \mapsto f(a_{m}) + B_{m + 1}.\]
	This makes \(\gr\) a functor between these two categories.
	% TODO: Try to typeset the gr f: grA \to gr B inline
	% TODO: Do I want to define (grading-), filtration-respecting maps? No. But should I? unsure.
\end{definition}

\begin{mdframed}
	Part about one direction of the f.t. being easy. Talk about why the other isn't (also how in the dual case it makes it hard to define a map knots to chord diagrams).
\end{mdframed}

The fundamental theorem of Vassiliev invariants allows us to establish a relationship between knots and chord diagrams, or equivalently, Vassiliev invariants and weight systems. It's not a-priori obvious how Theorem \ref{thm:fundamental-theorem-of-vassiliev-invariants} does this, but is apparent in the following equivalent reformulation.




The part due to Kontsevich is very involved. Kontsevich defined a knot invariant called the Kontsevich integral which is an incredibly fascinating object. We skip many of the details.



\begin{remark}
	\begin{mdframed}
		It is important to note that a-priori, we have no well-defined map \(\mathcal{K} \to \mathcal{A}\) that respects the singular knot filtration. We do have such a map for singular knots \(\singularknots \to \mathcal{A}\), but to extend this to \(\mathcal{K}\), we need to be sure that a linear combination in \(\mathcal{K}\) coming from resolving a singular knot can only be done in one way, or at least by singular knots with only one type of chord diagram. This fact is provided by the Kontsevich integral.
	\end{mdframed}
\end{remark}
...
\begin{definition}
	...
	A universal Vassiliev invariant is a map
	\[U : \mathcal{K} \to X\]
	to some space \(X\), such that any Vassiliev invariant can be factored through \(U\). For any Vassiliev invariant \(v\), there is a linear functional \(f\) on \(X\) such that \(v = f \circ U\).
\end{definition}


\begin{remark}
	Definition \ref{def:universal-vassiliev-invariant} is also known to some as a (homomorphic) expansion, for example \cite{finite-type-invariants-of-w-knotted-objects-I}, and related to the concept of a formality map.
	% TODO: find a reference for formality.
\end{remark}

\begin{theorem}[Universal Vassiliev Invariant]
