...
To illustrate this point, we look at a construction called the dual filtered bialgebra of \(\mathcal{K}\). The dual filtered bialgebra construction makes a filtered bialgebra whose \(m\)th filtered component is the set of functionals in \(\mathcal{K}^{\ast}\) that vanish on \(\mathcal{K}_{m + 1}\). For \(\mathcal{K}\), this coincides with the Vassiliev invariants.
...
\begin{definition}
	...
	\[0 = \mathcal{V}_{0} \subset \mathcal{V}_{1} \subset \mathcal{V}_{2} \subset \cdots, \qquad \mathcal{V} = \bigcup_{m = 0}^{\infty} \mathcal{V}_{m}.\]
...
\begin{proof}
	\begin{mdframed}
		Indeed, the Vassiliev invariants are those functionals in \(\mathcal{K}^{\ast}\) that vanish on ... . Furthermore the dual to the coproduct in \(\mathcal{K}\) is the product in \(\mathcal{V}\), and vice versa.

		One we know this, we can use the textbook fact that the dual of a descending filtered bialgebra is an ascending filtered bialgebra \cite{introduction-to-vassiliev-invariants} appendix. Alternatively, *move remark 1.3.14 here*.
	\end{mdframed}
\end{proof}
