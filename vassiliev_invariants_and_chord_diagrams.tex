...
\begin{mdframed}
	...
	In Section \ref{sec:the-bialgebras-of-knots-and-chord-diagrams}, with the context in mind, we introduce the main players in this theory.
\end{mdframed}
...
\section{The bialgebras of knots and chord diagrams}
\label{sec:the-bialgebras-of-knots-and-chord-diagrams}
...
\begin{proposition}
	\label{prop:singular-knots-filtered-bialebra}
...
\begin{lemma}[Willerton]
	...
	% TODO: Problem, didn't we mod out by the diff* relation...
\end{lemma}
...
\begin{proof}
	...
	The inductive step is as follows. Let \(k^{\bullet} \in \singularknots_{n + 1}\). Let \(x\) denote a specific singular point of \(k^{\bullet}\), and \(J\) denote all the other singular points of \(K\). So all singular points of \(k^{\bullet}\) are \(S = J \sqcup \{x\}\), and let us examine
	\[
		\sum_{\mathclap{I \subset J \sqcup \{x\}}}
	...
	\end{align*}
	\begin{align*}
		\Delta(\delta^{n + 1}(k^{\bullet}))
		& = \Delta(\delta^{n} \delta^{\{x\}}(k^{\bullet}) )\\
		& = \Delta(\delta^{n} (k^{\bullet +})) - \Delta(\delta^{n}(k^{\bullet -}) )\\
		& = \sum_{I \subset J}
			\mu^{\bar{I}}\delta^{I}(k^{\bullet+})
			\otimes
			\mu^{\bar{I}}\delta^{I}(k^{\bullet+})
			-
			\sum_{I \subset J} \mu^{\bar{I}}\delta^{I}(k^{\bullet-})
			\otimes
			\mu^{\bar{I}}\delta^{I}(k^{\bullet-}) \\
	\end{align*}
	On the other hand,
...
\begin{proofof}{Proposition \ref{prop:singular-knots-filtered-bialebra}}
...
\begin{remark}
	There is one techincallity. For the above lemma and proof we must forget the \ref{eq:codiff} relation on \(\singularknots_{n}\) (or work in the appropriate lift) as we wish to resolve with respect to specific double points. This provides no mathematical difficulty, so we chose not to do the reader the disservice of altering the notation.
\end{remark}

The algebra strucutre on knots is heavily related to an algebra of chord diagrams. Recall the \ref{eq:4T} and \ref{eq:1T} relations from the previous section. Just like with the \ref{eq:diff} and \ref{eq:codiff} relations, instead of studying functions on chord diagrams satisfying \ref{eq:4T} and \ref{eq:1T}, we prefer to study functions on spaces quotiented by relations on the objects.
