...
\begin{proofof}{Theorem \ref{thm:fundamental-theorem-restated} (Vassiliev)}
	...
	\begin{mdframed}
		We also need to show here that \(\gr \mathcal{A} \cong \mathcal{K}\) is true as a bialgebra. The product is easy, the coproduct shouldn't be too hard.
	\end{mdframed}

	The isomorphism respects the algebra structure as \(a_{1} \hash a_{2}\) is sent to \([k_{a_{1} \hash a_{2}}] \in \mathcal{K}_{m + n} / \mathcal{K}_{m + n + 1}\) that comes from the resolution of a singular knot \(k^{\bullet}_{a_{1} \hash a_{2}}\) with chord diagram \(a_{1} \hash a_{2}\). But likewise \(a_{1}\) and \(a_{2}\) map to \([k_{a_{1}}]\) and \([k_{a_{2}}]\) which are resolutions of singular knots \(k^{\bullet}_{a_{1}}\) and \(k^{\bullet}_{a_{2}}\). The induced operation from the connect sum \(\mathcal{K}_{m}/\mathcal{K}_{m + 1} \otimes \mathcal{K}_{n}/\mathcal{K}_{n + 1} \to \mathcal{K}_{m + n}/\mathcal{K}_{m + n + 1}\) takes these to \([k_{a_{1}} \hash k_{a_{2}}]\), which comes from the resolution of \(k^{\bullet}_{a_{1}} \hash k^{\bullet}_{a_{2}}\). These singular knots may not a-priori be the same, but certainly they have the same chord diagram. As a result, by the crossing-change argument they can be chosen to be the same, hence their resolutions are the same.

	\begin{mdframed}
		We sketch the argument in the dual case too. A \(v \in \mathcal{V}_{m}\) vanishes on \(\mathcal{K}_{m + 1}\)...
	\end{mdframed}
	\phantom{.}
\end{proofof}
...
\begin{proofof}{Theorem \ref{thm:fundamental-theorem-restated} (Kontsevich)}
	...
	In the dual case, we wish to prove that
...
The name `universal Vassiliev invariant' is indeed justified.

\begin{theorem}
	If \(Z\) is a universal Vassiliev invariant, than every Vassiliev invariant factors through \(Z\).
\end{theorem}

\begin{mdframed}
	Why the reformulation was a reformulation.
\end{mdframed}
