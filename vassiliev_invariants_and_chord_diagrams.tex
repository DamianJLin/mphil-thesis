...
\end{remark}
% TODO: (Zsuzsi) Also, tangential points. (Not sure you need an enumerate environment here)

A singular knot with one double point is very close to two other knots, one where it's replaced by a positive crossing and one by a negative. % TODO: (Zsuzsi) This sentence makes a great point but it's not grammatically sound
If the conditions are right, we can extend a knot invariant to an invariant of singular knots by ``taking its derivative''. %TODO (Zsuzsi): Taking the difference of these two values, called the derivateive? (To connect to previous point)
%TODO: (Zsuzsi) Make it clear that in general the derivative of an $m$-singular knot invariant is an $(m+1)$-singular knot invariant. 
\begin{definition}
...
Rather than thinking about functions on knots satisfying certain relations, the modern view of this subject takes the philosophy of imposing relations on the objects directly. %TODO: (Zsuzsi) I assume you talk about the classical view in the Intro, so this contrast makes sense?

\begin{definition}
	\label{def:differentiable-knot}
	Define \(\mathcal{K}_{m} = \operatorname{span}(\{m\text{-singular knots}\})\) 
    %TODO: (Zsuzsi) Specify what kind of span (linear over $\Q$?)
...
\begin{remark}
	The derivative operation and the \ref{eq:diff} relation are dual to the boundary operation and the \ref{eq:codiff} relation. For example, a differentiable invariant of knots is the same as a invariant of knots in \(\mathcal{K}_{m}\). %TODO: (Zsuzsi) This might deserve to be deminstrated on a small example. %TODO: (Zsuzsi) Worth noting that any invariant of actual (non-singular) knots is differentiable.
\end{remark}
...
The Vassiliev skein relation extends a knot via its derivative, or chooses a value on \((m + 1)\)-singular knots to agree with the difference of values on its boundary. %TODO: (Zsuzsi) I would move this sentence to before the skein relation: otherwise readers might get stuck trying to figure out the difference between the derivative and the Vassiliev skein relation before getting to this sentence.

\begin{definitions}
	\label{def:vassiliev-invariant} %TODO: (Zsuzsi) Insert an intro sentence so (a) and (b) are aligned
	\begin{enumerate}
		\item A knot invariant \(V\) is a \textbf{Vassiliev invariant} of order (or type) \(m\) if when extended to singular knots via the Vassiliev skein relation, there is an integer \(m\) such that %TODO: (Zsuzsi) Fix the logic here (if it's order $m$ then it's not "there exist" $m$).
	...
	\end{enumerate} %TODO: (Zsuzsi) Highest or lowest?
\end{definitions}
...
In the next section we will look at equivalent formulation of the fundamental theorem, and then in Chapter \ref{ch:combinatorial-integration_of_weight_systems} we present some progress towards Conjecture \ref{conj:strong-fundamental-theorem}.
...
\begin{mdframed}
	...
	We may take dual and end up with relations on chord diagrams instead. This gives us an algebra \(\mathcal{A}\) which we also call chord diagrams. (Are we *free* to choose values on these? It feels like this is the point, but I keep hearing that it's open. What's the correct thing to say? Potentially there's a technical detail here.) Either way.
...
The point of the fundamental theorem of Vassiliev invariants is that it allows us to establish the following relationship between Vassiliev invariants and weight systems.

Just like with the \ref{eq:diff} and \ref{eq:codiff} relation, 
